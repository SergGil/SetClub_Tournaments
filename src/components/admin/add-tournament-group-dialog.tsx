 ...
 * group picker).
 *
 * For a DOUBLES tournament the group's roster is entered as pairs (not loose
 * individual players) - the pairing here is already fully decided (e.g. the
 * known group-stage winners advancing into a "Гра за 1-3 місце" playoff), so
 * submitting immediately generates the full round robin between those exact
 * teams, tagged with the group's name, instead of leaving the admin to add
 * every match one at a time via "Додати матч".
 */
export function AddTournamentGroupDialog({
  ...
  /** DOUBLES tournaments enter pairs; every other format keeps the plain player picker. */
  isDoubles: boolean;
}) {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = isDoubles
        ? await createTournamentGroupWithPairsAction(
            tournamentId,
            name,
            pairSlots.filter((s) => s.a && s.b).map((s): [string, string] => [s.a, s.b]),
          )
        : await createTournamentGroupAction(tournamentId, name, selected);
      if (result?.error) {

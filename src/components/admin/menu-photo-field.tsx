 ...
 * NewsPhotoField (see docs/PHOTOS.md) - a menu item's photo is uploaded
 * before the surrounding form is even submitted, both because there's no
 * item id yet to key it by and because routing the file through the
 * create/update Server Action would hit Next's default 1MB body limit.
 */
async function uploadMenuPhoto(file: File): Promise<{ key?: string; error?: string }> {
  const presignRes = await fetch("/api/menu/photo-presign", {
    method: "POST",
...
/**
 * `photoKey` - "" when nothing new was uploaded (the item's existing photo,
 ...
 */
export function MenuPhotoField({ initialPhotoUrl }: { initialPhotoUrl?: string | null }) {
  ...
  async function handleFile(file: File) {
    ...
    const result = await uploadMenuPhoto(compressed);
    setUploading(false);
  ...
  return (
    <div className="flex flex-col gap-2">
      <Label htmlFor="menu-item-photo">Фото (опційно)</Label>
      {preview && (

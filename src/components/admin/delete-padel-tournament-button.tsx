...
import { deletePadelTournamentAction } from "@/lib/actions/padel-tournaments";
...
/** Padel twin of delete-tournament-button.tsx. */
export function DeletePadelTournamentButton({
  id,
  name,
  completedMatchCount,
}: {
  ...
  const [state, formAction] = useActionState(deletePadelTournamentAction, initialState);
        ...
        <form action={formAction}>
          ...
          {needsDeleteConfirmation && (
            <div className="mt-3 flex flex-col gap-1.5">
              <Label htmlFor="padel-tournament-delete-confirm" className="text-sm">
                Введіть <span className="font-semibold">{DELETE_CONFIRM_WORD}</span>, щоб
                підтвердити
              </Label>
              <Input
                id="padel-tournament-delete-confirm"
                value={confirmText}

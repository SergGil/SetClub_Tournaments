...
export type TeamWithMembers = { id: string; name: string; members: RosterPlayer[] };
...
function AddTeamDialog({
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await createTeamAction(tournamentId, name, selected);
      if (result?.error) {
    ...
    >
      ...
      <DialogContent>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="new-team-name">Назва команди</Label>
            <Input
              id="new-team-name"
              value={name}
...
function EditTeamDialog({
  ...
  // Every current tournament participant is a valid pick, plus this team's
  // own current members even if one happens to be missing from the roster
  // prop for some reason - keeps their badge visible instead of silently
  // dropping them from the picker's candidate list.
  const pickable = [...participants, ...team.members.filter((m) => !participants.some((p) => p.id === m.id))];

  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    startTransition(async () => {
      const result = await updateTeamAction(tournamentId, team.id, name, selected);
      if (result?.error) {
        toast.error(result.error);
      } else {
        setOpen(false);
    ...
    >
      ...
      <DialogContent>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="edit-team-name">Назва команди</Label>
            <Input
              id="edit-team-name"
              value={name}
              onChange={(e) => setName(e.target.value)}
...
function DeleteTeamButton({ tournamentId, team }: { tournamentId: string; team: TeamWithMembers }) {
  const [pending, startTransition] = useTransition();

  function confirmDelete() {
    startTransition(async () => {
      const result = await deleteTeamAction(tournamentId, team.id);
...
/**
 * Opt-in roster split into 2-4 player teams for MIXED tournaments (see
 * docs/TOURNAMENT_TEAMS.md) - only rendered by the admin page when
 * `format === "MIXED"`. A team on its own doesn't do anything yet; it only
 * becomes useful once paired against another team in a tie (TournamentTies,
 * rendered right below this section).
 */

...
import { updatePadelTournamentGroupAction, updatePadelTournamentGroupPairsAction } from "@/lib/actions/padel-tournaments";
...
/** Padel twin of edit-tournament-group-dialog.tsx. */
export function EditPadelTournamentGroupDialog({
  tournamentId,
  groupId,
  groupName,
  memberIds,
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      const result = await updatePadelTournamentGroupAction(tournamentId, groupId, name, selected);
      if (result?.error) {
        toast.error(result.error);
      } else {
        setOpen(false);
      ...
      <DialogContent>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="edit-padel-group-name">Назва групи</Label>
            <Input
              id="edit-padel-group-name"
              value={name}

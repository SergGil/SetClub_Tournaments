...
/** Padel twin of create-tie-dialog.tsx. */
export function CreatePadelTieDialog({
  tournamentId,
  teams,
}: {
  tournamentId: string;
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await createPadelTieAction(tournamentId, teamAId, teamBId, label);
      if (result?.error) {
    ...
    >
      ...
      <DialogContent>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="padel-tie-label">Мітка (опційно)</Label>
            <Input
              id="padel-tie-label"
              value={label}

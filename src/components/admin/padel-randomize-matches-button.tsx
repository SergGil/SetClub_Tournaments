...
export function PadelRandomizeMatchesButton({
  ...
  async function startDraw() {
    ...
    const result =
      strategy === "CUSTOM_GROUPS"
        ? await drawPadelDoublesGroupsAction(
            tournamentId,
            fixedPairs,
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      >
        ...
        {phase === "intro" && needsGroupCountInput && (
          <div className="flex flex-col gap-2">
            <Label htmlFor="padel-doubles-group-count">Кількість груп</Label>
            <Select
            ...
            >
              <SelectTrigger id="padel-doubles-group-count" className="w-full">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        {phase === "intro" && strategy === "CUSTOM_GROUPS" && canOfferGroupPlayoff && (
          <div className="flex flex-col gap-2 rounded-lg border p-3">
            <div className="flex items-start gap-2">
              <Checkbox
                id="padel-doubles-with-playoff"
                checked={withPlayoff}
                onCheckedChange={(checked) => setWithPlayoff(checked === true)}
                className="mt-0.5"
              />
              <Label htmlFor="padel-doubles-with-playoff" className="font-medium">

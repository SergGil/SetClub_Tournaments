...
async function uploadOne(tournamentId: string, file: File): Promise<{ error?: string }> {
  const presignRes = await fetch("/api/padel-photos/presign", {
    method: "POST",
  ...
  return confirmPadelPhotoUploadAction(tournamentId, key);
}

/** Padel twin of photo-upload-dialog.tsx. */
export function PadelPhotoUploadDialog({ tournamentId }: { tournamentId: string }) {

...
const { createTeamActionMock, updateTeamActionMock, deleteTeamActionMock } = vi.hoisted(() => ({
  createTeamActionMock: vi.fn(async () => ({})),
  updateTeamActionMock: vi.fn(async () => ({})),
  deleteTeamActionMock: vi.fn(async () => ({})),
}));
vi.mock("@/lib/actions/teams", () => ({
  createTeamAction: createTeamActionMock,
  updateTeamAction: updateTeamActionMock,
  deleteTeamAction: deleteTeamActionMock,
}));
...
describe("TournamentTeams (create)", () => {
  it("shows the empty state with no teams yet", () => {
    render(<TournamentTeams tournamentId="t1" teams={[]} participants={participants} />);
  ...
  it("creates the team with the picked players and closes on success", async () => {
    ...
    await waitFor(() =>
      expect(createTeamActionMock).toHaveBeenCalledWith("t1", "Команда 1", ["p1", "p2"]),
    );
  ...
  it("shows a toast and keeps the dialog open when creation fails", async () => {
    createTeamActionMock.mockResolvedValueOnce({ error: "Гравець уже в іншій команді цього турніру" });
    const user = userEvent.setup();
    render(<TournamentTeams tournamentId="t1" teams={[]} participants={participants} />);
...
describe("TournamentTeams (existing teams)", () => {
  const teams = [
    {
      id: "team1",
      name: "Команда 1",
  ...
  it("lists the team's name and members", () => {
    render(<TournamentTeams tournamentId="t1" teams={teams} participants={participants} />);
  ...
  it("saves an edited member list", async () => {
    ...
    await waitFor(() =>
      expect(updateTeamActionMock).toHaveBeenCalledWith("t1", "team1", "Команда 1", ["p1", "p2", "p3"]),
    );
  });
  ...
  it("deletes the team after confirming", async () => {
    ...
    await waitFor(() => expect(deleteTeamActionMock).toHaveBeenCalledWith("t1", "team1"));
  });
});

...
describe("RandomizeMatchesButton (За групами)", () => {
  ...
  it("reveals grouped teams one by one, then commits per-group matchups tagged with their group", async () => {
    ...
    // Only 1 pre-existing group (4 players) here - withPlayoff needs exactly
    // 2 groups of 8, so the checkbox stays hidden in favor of the hint.
    ...
    expect(drawDoublesGroupsActionMock).toHaveBeenCalledWith("t1", [], undefined, false);
    expect(await screen.findByText("Пар сформовано: 0 / 2")).toBeInTheDocument();
  ...
  it("offers withPlayoff for a fresh 2-group split of exactly 16 players and threads it through to draw", async () => {
    ...
    expect(drawDoublesGroupsActionMock).toHaveBeenCalledWith("t1", [], 2, true);
  });

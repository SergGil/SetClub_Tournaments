...
describe("EditTournamentGroupDialog", () => {
  ...
  it("saves the renamed group and updated member list", async () => {
    ...
    await waitFor(() =>
      expect(updateTournamentGroupActionMock).toHaveBeenCalledWith("t1", "g1", "Фінальна група", [
        "p1",
        "p3",
      ]),
    );
  ...
  it("removing a pre-selected member's badge excludes them from the save", async () => {
    ...
    await waitFor(() =>
      expect(updateTournamentGroupActionMock).toHaveBeenCalledWith("t1", "g1", "Плейофф", ["p2"]),
    );
  });

  it("shows a toast and keeps the dialog open when the update fails", async () => {
    updateTournamentGroupActionMock.mockResolvedValueOnce({ error: "Групу не знайдено" });
    const user = userEvent.setup();
    render(
      <EditTournamentGroupDialog
        tournamentId="t1"

...
const { prismaMock } = vi.hoisted(() => ({
  prismaMock: {
    padelTournamentTeam: { findMany: vi.fn() },
    padelTournamentTie: { create: vi.fn(), delete: vi.fn(), findUnique: vi.fn() },
    padelMatch: { create: vi.fn() },
  },
}));
...
import { createPadelRubberAction, createPadelTieAction, deletePadelTieAction } from "@/lib/actions/padel-ties";
...
describe("createPadelTieAction", () => {
  it("rejects picking the same team twice", async () => {
    const result = await createPadelTieAction("t1", "teamA", "teamA");
    expect(result.error).toBeDefined();
    expect(prismaMock.padelTournamentTie.create).not.toHaveBeenCalled();
  });

  it("rejects a team that doesn't belong to this tournament", async () => {
    prismaMock.padelTournamentTeam.findMany.mockResolvedValueOnce([{ id: "teamA" }]);
    const result = await createPadelTieAction("t1", "teamA", "teamB");
    expect(result.error).toBeDefined();
    expect(prismaMock.padelTournamentTie.create).not.toHaveBeenCalled();
  });

  it("creates the tie with an optional label", async () => {
    prismaMock.padelTournamentTeam.findMany.mockResolvedValueOnce([{ id: "teamA" }, { id: "teamB" }]);
    prismaMock.padelTournamentTie.create.mockResolvedValueOnce({ id: "tie1" });

    const result = await createPadelTieAction("t1", "teamA", "teamB", "Тур 1");

    expect(result.error).toBeUndefined();
    expect(prismaMock.padelTournamentTie.create).toHaveBeenCalledWith({
  ...
  it("stores no label at all when left blank", async () => {
    ...
    await createPadelTieAction("t1", "teamA", "teamB");
...
describe("deletePadelTieAction", () => {
  it("reports a missing tie instead of deleting anything", async () => {
    prismaMock.padelTournamentTie.findUnique.mockResolvedValueOnce(null);
    const result = await deletePadelTieAction("t1", "tie1");
    expect(result.error).toBeDefined();
    expect(prismaMock.padelTournamentTie.delete).not.toHaveBeenCalled();
  });

  it("deletes the tie - rubbers survive via the schema's own SetNull FK, no extra cleanup needed here", async () => {
    prismaMock.padelTournamentTie.findUnique.mockResolvedValueOnce({ tournamentId: "t1", label: "Тур 1" });

    const result = await deletePadelTieAction("t1", "tie1");

    expect(result.error).toBeUndefined();
    expect(prismaMock.padelTournamentTie.delete).toHaveBeenCalledWith({ where: { id: "tie1" } });
  });
});

describe("createPadelRubberAction", () => {
  function mockTie(teamAIds: string[], teamBIds: string[]) {
    prismaMock.padelTournamentTie.findUnique.mockResolvedValueOnce({
      tournamentId: "t1",
  ...
  it("reports a missing tie instead of creating a match", async () => {
    prismaMock.padelTournamentTie.findUnique.mockResolvedValueOnce(null);
    const result = await createPadelRubberAction(initialState, rubberFormData({ sideAPlayerIds: "p1", sideBPlayerIds: "p2" }));
    expect(result.error).toBeDefined();
  ...
  it("rejects a side-A player who isn't a member of team A, even if they're a real tournament participant", async () => {
    ...
    formData.set("sideAPlayerIds", "b1");
    formData.set("sideBPlayerIds", "b2");

    const result = await createPadelRubberAction(initialState, formData);

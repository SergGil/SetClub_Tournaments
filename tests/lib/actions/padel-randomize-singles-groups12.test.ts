...
describe("drawPadelGroups12PlayoffAction", () => {
  ...
  it("errors when there aren't exactly 12 participants", async () => {
    ...
    prismaMock.padelTournamentParticipant.findMany.mockResolvedValueOnce(makeRoster(4, 7));
    const result = await drawPadelGroups12PlayoffAction("t1");
    expect(result.ok).toBe(false);
  ...
  it("draws 4 empty A-D baskets, all 12 players, and 12 matchups", async () => {
    prismaMock.padelTournament.findUnique.mockResolvedValueOnce({ format: "SINGLES" });
    prismaMock.padelTournamentParticipant.findMany.mockResolvedValueOnce(makeRoster(4, 8));
...
describe("commitPadelGroups12PlayoffAction", () => {
  ...
  it("errors for a non-singles tournament", async () => {
    prismaMock.padelTournament.findUnique.mockResolvedValueOnce({ format: "DOUBLES" });
    const result = await commitPadelGroups12PlayoffAction("t1", groupAssignment, matchups, false);
  ...
  it("rejects a group assignment outside 1-4", async () => {
    ...
    prismaMock.padelMatch.count.mockResolvedValueOnce(0);
    prismaMock.padelTournamentParticipant.findMany.mockResolvedValueOnce(makeRoster(4, 8));
    const result = await commitPadelGroups12PlayoffAction(
      "t1",
  ...
  it("rejects a matchup referencing a player outside the roster", async () => {
    ...
    const result = await commitPadelGroups12PlayoffAction("t1", groupAssignment, badMatchups, false);
    expect(result.error).toBe("Некоректні дані розіграшу");
  });
  ...
  it("persists group assignments, 12 group matches, 18 bracket placeholders, and 36 advancement rows", async () => {
    ...
    expect(txMock.padelTournamentParticipant.update).toHaveBeenCalledTimes(12);

    const matchCreateCalls = txMock.padelMatch.createMany.mock.calls;
    ...
    expect(txMock.padelMatchPlayer.createMany).toHaveBeenCalledTimes(1);
    expect(txMock.padelMatchPlayer.createMany.mock.calls[0][0].data).toHaveLength(24);

    expect(txMock.padelMatchAdvancement.createMany).toHaveBeenCalledTimes(1);
    const advancementRows = txMock.padelMatchAdvancement.createMany.mock.calls[0][0].data;
    expect(advancementRows).toHaveLength(36);

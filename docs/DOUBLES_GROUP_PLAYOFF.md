...
## Мобільний застосунок — свідомо поза межами цього кроку

`/api/v1/tournaments/[id]/randomize/doubles/{draw-groups,commit-groups}/route.ts` (і padel-
дзеркало) отримали `withPlayoff: false` явним літералом — мобільний рандомайзер ще не пропонує цей
чекбокс. Підключення туди — природний наступний крок.

## Побічний фікс, знайдений під час роботи

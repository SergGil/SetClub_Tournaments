...
## 2026-09-15 — Нова вкладка "Школа" в розділі Теніс (набір дітей від 4 років)

Додано публічну сторінку `/tennis/school` — набір у школу тенісу: банер-фото, чотири вікові групи
(MIMI 4-5, MINI 6-7, KIDS 8-9, TEENS 12-14) з фото та підписами, відео зі школи, і блок запису за
телефоном клубу. Використано ті самі стилістичні конвенції, що й на `/tennis/coaches` і
`/tennis/pricing` (full-bleed секція, `Image` зі статичними асетами з `public/`).

Посилання додано в `NAV_LINKS` (`src/lib/site.ts`) між "Теніс" і "Ціни", у сайтмап
(`src/app/sitemap.ts`), і карткою-лінком на `/tennis` поруч із "Наші тренери".

Асети лежали в `public/school/` (`school.jpg`, `mimi.jpg`, `mini.jpg`, `kids.jpg`, `teens.jpg`,
відео) — перейменував відео з `school enrollment.mp4` на `school-enrollment.mp4` (прибрав пробіл
з імені файлу, щоб уникнути проблем з URL-кодуванням).

**Файли**: `src/app/tennis/school/page.tsx` (новий), `src/app/tennis/page.tsx`, `src/lib/site.ts`,
`src/app/sitemap.ts`, `public/school/*`.

**Після живого перегляду виправлено 3 регресії**, знайдені користувачем на проді:

1. Банер-фото було обрізане знизу (зникав напис "НАБІР У ШКОЛУ ТЕНІСУ") — контейнер мав
   `aspect-[4/3] sm:aspect-[16/9]` (альбомний), а фото насправді 1080×1440 (портретне, 3:4).
   Замінено на `aspect-[3/4]` у вужчій центрованій колонці (`max-w-sm sm:max-w-md`) замість
   розтягування на всю ширину.
2. Відео не вміщалось на екран (портретний формат розтягувався на всю ширину
   контент-колонки — до ~1900px висоти). Обмежено `max-h-[75vh]` з `w-auto`, центровано.
3. Додавання 11-ї вкладки в `NAV_LINKS` зробило рядок навігації досить широким, щоб при
   ефективній ширині viewport ~1024-1220px (типово для Windows зі масштабуванням дисплея,
   напр. 1366×768 @125% ≈ 1093px) кнопка "Вийти" в правому кластері переносилась на новий
   рядок — той самий клас бага, що вже раз виправляли (див. max-w-6xl → max-w-7xl вище по
   історії), але той фікс діє лише від 1280px; нижче — ширина хедера дорівнює ширині
   viewport. Замість чергового підбору max-w перенесено поріг показу інлайн-навігації та
   бургер-меню з `lg:` (1024px) на `xl:` (1280px) — у проміжку тепер бургер-меню замість
   рядка з 11 посилань. Перевірено Playwright-репродукцією на ширинах 1000-1440px до і
   після фіксу.

**Файли**: `src/app/tennis/school/page.tsx`, `src/components/nav-links.tsx`,
`src/components/nav.tsx`.

**Виправлення №3 виявилось неповним** — той самий баг зі зникненням "Вийти" повернувся на
живому сайті для суперадміна: `xl:` (1280px) — це саме той поріг, де `max-w-7xl` вже не давав
запасу (виміряно, 11 посилань + бейдж + ім'я), АЛЕ суперадмін бачить 12-те посилання
("Адмін-панель", `ADMIN_NAV_LINK`), яке я не врахував у попередньому тестовому відтворенні.
Разом це вимагає ~1319px, а `max-w-7xl` дає лише 1280px — тобто ширина екрана взагалі не мала
значення, бо ширина хедера впирається в `max-w`, а не в viewport. Замінено `max-w-7xl` →
`max-w-[92rem]` (1472px, ~150px запасу) і поріг показу інлайн-навігації/бургера/бейджа з `xl:`
на кастомний `min-[1400px]` (щоб точка вмикання інлайн-рядка завжди співпадала з шириною, де
рядок реально вміщується — інакше лишається "мертва зона" між точкою вмикання іменованого
брейкпоінта і точкою, де контент справді влазить). Перевірено Playwright-репродукцією з 12
посиланнями на діапазоні 900-2560px.

**Файли**: `src/components/nav.tsx`, `src/components/nav-links.tsx`.

## 2026-09-15 — Швидкі посилання на Кава/Теніс/Падел у хедері адмін-панелі

За запитом користувача: суперадмін (чи домен-адмін), перебуваючи всередині `/admin/*`, тепер
бачить у хедері поруч з "Адмін-панель" короткі посилання назад на публічні хаби — "Теніс",
"Кава", "Падел" — щоб не робити гак через головну сторінку. Кожне посилання показується лише
тому адміну, який дійсно адмініструє цей домен (той самий принцип, що вже використовувався для
Падел-перемикача фону — `hasPadelAdminAccess`); додано симетричний `hasTennisAdminAccess`.

Новий компонент `ShowOnAdminIfAuthorized` (`nav-home-hide.tsx`) навмисно вужчий за
`ShowOnHomeIfAuthorized` — спрацьовує лише на `/admin/*`, не на `/`, бо головна сторінка вже
показує всі три хаби як основний контент (triple-split.tsx); дублювати їх у хедері там не було
сенсу.

**Файли**: `src/components/nav-home-hide.tsx`, `src/components/nav.tsx`,
`tests/components/nav-home-hide.test.tsx`, `tests/components/nav.test.tsx`.

## 2026-09-14 — Виправлено передіснуючий vitest-флейк в edit-{tournament,padel-tournament}-group-dialog

Справжня причина знайдена (не просто "система під навантаженням"): в обох компонентах кнопка
"Зберегти"/"Скасувати" — це ОДНА кнопка, чия accessible name перемикається між "Зберегти" і
"Збереження…" тим самим `useTransition`-прапорцем `pending`, який також визначає, чи вже
відрендерилось поле підтвердження "ВИДАЛИТИ" (обидва стани оновлюються в одному
`startTransition`-колбеку). Тест чекав на поле підтвердження через `findByLabelText` (асинхронно,
з повтором), а одразу після цього брав кнопку через `getByRole` (синхронно, один раз) — і зрідка
(частіше під навантаженням, коли одразу кілька важких тестових файлів виконуються паралельно)
встигав спіймати кнопку ще з іменем "Збереження…" (disabled), хоча поле вже з'явилось. Відтворено
детерміновано через `--no-file-parallelism`: акцесибіліті-дерево на момент падіння показало кнопку
з іменем саме "Збереження…", не "Зберегти".

Виправлено заміною `getByRole` на `findByRole` для цієї кнопки — чекає, доки ім'я справді
повернеться до "Зберегти" (тобто `pending` дійсно очистився), замість покладатись на те, що обидва
стани оновлюються атомарно. Перевірено 5× поспіль парними прогонами обох файлів (раніше падало
регулярно) — тепер стабільно 16/16; повний `vitest run` — 184/184 файлів, 1807/1807 тестів.

**Файли**: `tests/components/admin/edit-tournament-group-dialog.test.tsx`,
`tests/components/admin/edit-padel-tournament-group-dialog.test.tsx`.

## 2026-09-14 — Прогонка застосунка на баги: виправлено e2e-набір (справжніх багів у продукті не знайдено)

За запитом "прогонка на можливі баги" пройшовся vitest + Playwright e2e + ручним обходом
публічних/адмінських сторінок. Знайдено й виправлено 3 реальні проблеми — усі в тестовій
інфраструктурі, жодна в самому продукті:

1. **`.env` і `.env.local` розходились** по `E2E_TEST_LOGIN_SECRET` (різні значення), а
   `playwright.config.ts` вантажив лише `.env` через голий `dotenv.config()` — тоді як `next dev`
   сам слідує пріоритету `.env.local` > `.env`. Через це e2e-специ логінились НЕПРАВИЛЬНИМ
   секретом і отримували 403 в `beforeEach`, валячи майже все в `admin-flows`/`groups12-playoff`/
   `match-flows`/`withdrawal-flow`. Виправлено: `config({ path: ".env.local", override: true })`
   після базового `config()`, дзеркалячи реальний пріоритет Next.js.
2. **`public-pages.spec.ts`** мав застарілі перевірки з часів ДО переходу на TripleSplit-хаб
   (`docs/HOMEPAGE.md`): шукав заголовок "SET.club" і кнопку "Загальний рейтинг" на `/`, яких
   давно нема (це тепер на `/tennis`, і "Загальний рейтинг" ніде вже не існує — на `/leaderboard`
   тепер "Загальна статистика"). Тести на 404 для невідомого турніру/гравця очікували статус 404,
   хоча `notFound()` в Next 16 з Cache Components віддає soft-404 (200 + `noindex`), якщо перевірка
   спрацьовує вже після старту стрімінгу — задокументовано в
   `node_modules/next/dist/docs/.../functions/not-found.md`. Тести оновлено відповідно до
   реальної поведінки; якщо хочете справжній HTTP 404 — перевірку треба виносити в `proxy`
   (Next 16 заміна middleware), окреме архітектурне рішення, не зроблено тут.
3. **`match-flows.spec.ts`** створював гравця і одразу шукав його ім'я в НЕфільтрованому списку
   `/admin/players` — а клуб уже має 40+ реальних гравців, список пагінований по 20, тож новий
   гравець рідко потрапляє на першу сторінку (той самий патерн, вже виправлений раніше в
   `withdrawal-flow.spec.ts`/`groups12-playoff.spec.ts`/`admin-flows.spec.ts`, просто не
   розповсюджений сюди). Останній тест того ж файлу (Рерандомайзер) не вводив слово "ВИДАЛИТИ" —
   а попередній тест уже зафіксував рахунок матчу, тож кнопка "Створити" лишалась заблокованою
   (той самий запобіжник, що й у `edit-tournament-group-dialog.tsx`). Обидва виправлено.

**Не підтверджено як баг продукту**: пара randomizer/tournament-creation прогонів падала з різними
таймаутами на різних кроках при повторних запусках — ознака нестабільної/повільної віддаленої
Neon-БД під навантаженням сьогодні (сам набір тестів вже має коментарі про це), а не детермінований
дефект коду. `tests/components/admin/edit-{tournament,padel-tournament}-group-dialog.test.tsx`
мають передіснуючий vitest-флейк (падають лише разом, у повному прогоні; окремо — проходять) —
не пов'язаний з роботою цієї сесії, залишено без змін.

**Файли**: `playwright.config.ts`, `e2e/public-pages.spec.ts`, `e2e/match-flows.spec.ts`.

## 2026-09-14 — Підпис і опис панелі на головній тепер опційні

У формі `/admin/home` "Підпис над назвою" (eyebrow) і "Опис" були обов'язковими полями — після
першого ж використання виявилось незручним. `HomePanelSettings.eyebrow`/`.description` стали
nullable в БД; `homePanelSettingsFormSchema` приймає порожнє значення й перетворює його на `null`
(той самий "порожній рядок → null" патерн, що й `MenuSection.tagline`). `getHomePanelSettings()`
тепер віддає рівно те, що збережено в рядку (включно з `null`), а не підставляє захардкоджений
дефолт замість свідомо очищеного поля — дефолт діє лише поки панель жодного разу не зберігали.
`TripleSplit` ховає підпис/опис, коли вони порожні; "Велика назва" лишається обов'язковою.

**Файли**: `prisma/schema.prisma` (+міграція `home_panel_optional_eyebrow_description`),
`src/lib/validation/home-panels.ts`, `src/lib/queries/home-panels.ts`,
`src/components/admin/home-panel-form.tsx`, `src/components/triple-split.tsx`.
...
(`pathname.startsWith("/padel")`), але посилання "Новини"/"Фото" в Padel-навігації ведуть на
club-wide `/news?hub=padel` і `/gallery?hub=padel` — не під `/padel/...`. Тому на цих сторінках
жоден route-клас не ставився, і правило в globals.css за замовчуванням показувало тенісний
фон-фото замість падельного (коли увімкнено відповідний перемикач). Виправлено: гвард тепер читає
той самий `?hub=` маркер, що й `useSectionLinks` у nav-links.tsx (звідти й скопійована логіка),
тож `/news?hub=padel` та `/gallery?hub=padel` тепер отримують `padel-route`.

**Файли**: `src/components/section-route-guard.tsx`.

## 2026-09-14 — Тепліша, менш чорно-біла кольорова палітра

Запит: зробити вигляд застосунку "трохи модернового" і тепліше, без повного редизайну (спершу
показав 3 повноцінні тенісні напрями — ґрунт/Вімблдон/хардкорт — як артефакт-макет, але
користувач попросив легший захід замість глобальної переробки).

Усі нейтральні кольори (`--background`, `--foreground`, `--card`, `--secondary`, `--muted`,
`--border` тощо) в `:root` і `.dark` отримали легкий теплий відтінок (oklch hue ~45-75) замість
чистого сірого (hue 0) — світла тема стала теплим кремовим/крейдяним, а не білим; темна —
теплим вугільно-коричневим, а не чистим чорним. Основний зелений (`--primary`) зсунуто тепліше
(hue 150→~138-142). Заразом виправлено залишковий сторонній синій у `--sidebar-primary` темної
теми (був не пов'язаний з рештою палітри) — тепер узгоджений із основним зеленим.

Додано ледь помітний теплий градієнт-підсвітку на `body` (замість суцільної заливки) — не показується,
коли увімкнено фон-фото (`bg-photo`/`bg-photo-padel`), бо ті правила все одно перекривають
`background-image`. `Card` отримала легку тінь (`shadow-sm`) для трохи більшої глибини.

**Файли**: `src/app/globals.css`, `src/components/ui/card.tsx`.

Додатково: `/tennis/pricing` і `/tennis/coaches` мали жорстко закодовані `bg-neutral-950`/`text-white`/
`border-zinc-800` тощо — єдині два місця в застосунку з таким хардкодом, тому вони не реагували ні
на нову теплу палітру, ні на світлу/темну тему взагалі. Замінено на токени теми (`bg-background`,
`text-foreground`, `bg-card`, `bg-muted`, `border-border`, `text-muted-foreground`), тепер обидві
сторінки узгоджені з рештою сайту.

**Файли**: `src/app/tennis/pricing/page.tsx`, `src/app/tennis/coaches/page.tsx`.

## 2026-09-12 — Виправлено порядок тай-брейку: очна зустріч знову першою для чистої нічиї двох

Запис від 2026-09-11 ("різниця геймів → виграні гейми → особиста зустріч") виявився не тим, чого
насправді хотів користувач — на реальному прикладі (дві пари по 4 очки, 18:13 і 16:16) різниця
геймів вирішувала МІСЦЕ РАНІШЕ за очну зустріч між ними в межах групи, хоча вони грали одне з
одним. Правильний намір: очна зустріч має вирішувати одразу, щойно тай-брейк звужується рівно до
двох команд/гравців (незалежно від того, наскільки різняться їхні гейми) — різниця геймів і
виграні гейми залишаються резервним критерієм лише коли: (а) ці двоє між собою не грали в цій
групі, або (б) у тай-брейку зав'язано 3+ учасники (де особиста зустріч нетранзитивна).

`sortTiedGroup`/`cascadeSort` (`src/lib/standings-sort.ts`) тепер перевіряють очну зустріч на
кожному рівні рекурсії, щойно група звужується до рівно двох — до, а не після числових критеріїв.

**Файли**: `src/lib/standings-sort.ts`, `tests/lib/standings-sort.test.ts`.
...
негрупованих учасників по групах, взагалі не знаючи, хто сіяний — сіяність враховувалась лише
всередині вже готової групи (пари сіяний+несіяний), тож чисто випадково всі сіяні гравці клубу
могли осісти в одній групі, а інша лишитись без жодного.

Тепер розподіл балансує одночасно за двома критеріями: сіяний юніт іде в групу з найменшою
кількістю вже призначених сіяних (кількість гравців — лише тай-брейк), несіяний юніт — навпаки,
за кількістю гравців (сіяні — тай-брейк, бо на них не впливає). У межах кожного з двох "кошиків"
спершу йдуть пари (щоб не перебивали баланс, потрапляючи саме в момент нічиєї), потім одинаки —
той самий прийом, що й у фіксі кількості гравців.

**Файли**: `src/lib/randomize-pairs.ts`, `src/lib/actions/randomize-doubles.ts`,
`tests/lib/randomize-pairs.test.ts`.

## 2026-09-11 — Розподіл парного турніру по групах міг дати непарні групи через fixed pair

`assignUngroupedDoublesToGroups` (`src/lib/randomize-pairs.ts`) розкидала учасників по групах
round-robin **по юнітах**, а не по гравцях — заздалегідь визначена пара (fixed pair) рахувалась як
1 юніт, так само як і кожен вільний гравець. При 1 парі + 14 вільних гравцях (15 юнітів на 2
групи) це давало розкид 8 юнітів проти 7 — і якщо пара опинялась серед "8 юнітів", та група
отримувала 9 реальних гравців проти 7 в іншій, хоча 16 гравців мали розділитись порівну по 8.
Наслідок був видно в самій турнірній таблиці: по одному "гравцю без пари" в кожній із двох груп
(непарна кількість не дозволяє розбити всіх на пари).

Виправлено на жадібний розподіл за фактичною кількістю гравців: кожен юніт (спершу пари, потім
одинаки — щоб пара не "перебивала" рівновагу саме в момент нічиєї між групами) іде в групу, де
зараз найменше гравців.

**Файли**: `src/lib/randomize-pairs.ts`, `tests/lib/randomize-pairs.test.ts`.

## 2026-09-09 — "Активність клубу" відкривається одразу на останньому місяці

Графіки "Матчів по місяцях"/"Турнірів по місяцях" на `/leaderboard` і `/padel/leaderboard`
відкривались проскрольними на найстаріший місяць зліва, тож щоб побачити актуальну активність
клубу, треба було одразу гортати вправо до кінця. `HorizontalScroller`
(`src/components/horizontal-scroller.tsx`, спільний для цих графіків і карток результатів/новин)
отримав необов'язковий проп `initialScroll?: "start" | "end"` (за замовчуванням "start" — старі
використання не змінили поведінки): для `"end"` смуга ставиться на правий край ще до першого
рендеру (`useLayoutEffect`, без видимого стрибка). Обидва `MonthlyBarChart` тепер передають
`initialScroll="end"`.

**Файли**: `src/components/horizontal-scroller.tsx`, `src/app/leaderboard/page.tsx`,
`src/app/padel/leaderboard/page.tsx`.

## 2026-09-09 — Тай-брейк у турнірних таблицях: різниця геймів → виграні гейми → особиста зустріч

Змінено пріоритет критеріїв ранжування в `sortRows`/`sortTiedGroup` (`src/lib/standings-sort.ts`)
— спільний модуль для всіх турнірних таблиць (одиночні/парні, теніс/падел, mini-групи плейоф,
"Хто виходить далі" в `bracket-advancement.ts`) і для нарахування Set Club очок
(`src/lib/rating/placement.ts`).

Раніше особиста зустріч перевірялась одразу після перемог/% перемог, ще до різниці геймів — тобто
могла "переважити" реальну числову перевагу за геймами, коли рівно двоє лишались у тай-брейку.
Тепер порядок каскадний і числові критерії йдуть першими: різниця геймів → сумарно виграні гейми
(нова сходинка — розрізняє, наприклад, 10:10 і 11:11, які раніше мали однакову різницю 0) → і лише
коли після обох цих кроків залишається рівно двоє з однаковими показниками — особиста зустріч
між ними. Для групи з 3+ учасників, що не розійшлася жодним числовим критерієм, особиста зустріч
не застосовується (нетранзитивна для 3+, як і раніше), і порядок визначається сіяністю/алфавітом.

Реальний наслідок для скріна "Група за 4-6 місце", який і був приводом: пара з рахунком 10:10
(різниця 0, як і в решти) тепер опускається на 3-тє місце в групі, бо виграла менше геймів
сумарно (10) ніж інші дві пари (по 11) — а місця 1-2 між тими двома вирішує їхня особиста зустріч
у цій же групі (або алфавіт, якщо вони саме тут не грали одне з одним).

**Файли**: `src/lib/standings-sort.ts`, `tests/lib/standings-sort.test.ts`.

## 2026-09-01 — Мобільний застосунок: три баги в щойно доданому date-picker/фото

Ручний код-рев'ю щойно запушеного батчу (`docs/MOBILE_APP.md`) виявив три реальні баги, до
першого ручного тестування на пристрої:

- **DateField зберігав не той день** — `onChange` конвертував `selectedDate` (Date на місцевій
  півночі обраного дня) через `toISOString()`, що в будь-якому додатньому UTC-зсуві (Україна
  включно) відкочує на попередній день. Обрати "1 червня" зберігало "31 травня" — торкалось усіх
  трьох дато-полів (турнір/матч/раббер). Виправлено: рядок `YYYY-MM-DD` будується з локальних
  Y/M/D-компонентів Date, не через `toISOString()`.
- **iOS-пікер не закривався** — `display="inline"` не має власної кнопки закриття, а `setShow(false)`
  раніше викликався лише на Android. Додано кнопку "Готово" для iOS і toggle при повторному
  натисканні на поле.
- **Завантаження фото могло падати на iPhone** — фото з бібліотеки Photos часто в форматі HEIC,
  якого немає в серверному allowlist (`ALLOWED_PHOTO_CONTENT_TYPES`), а `asset.mimeType`
  передавався як є — presign-запит впав би з 400. Додано `preferredAssetRepresentationMode:
  Compatible` в обидва виклики `launchImageLibraryAsync` (фото турніру, фото новини), плюс
  `guessContentType` тепер ніколи не пропускає непідтримуваний mimeType далі презайну.

**Файли**: `mobile/src/components/date-field.tsx`, `mobile/src/lib/photo-upload.ts`,
`mobile/src/app/(tabs)/tournaments/[id]/index.tsx`, `mobile/src/features/news/news-form.tsx`.
...
`tournaments/[id]/randomize.tsx` (кнопка "Жеребкування" на картці турніру, окрім MIXED-турнірів —
ті керуються через Команди/зустрічі): одиночний round robin (ALL/SEEDED_SPLIT, одна дія без
прев'ю) і парне жеребкування (draw → прев'ю пар і матчів → commit), обидва з тим самим
cascade-confirm UX при заміні вже завершених матчів, що й в інших флоу (withdraw/delete/reset).

Свідомо не реалізовано зараз: CUSTOM_GROUPS/GROUPS_12_PLAYOFF (одиночні) і "За групами" (парні) —
три довші, менш вживані стратегії з тим самим draw/commit патерном, коли знадобляться.

**Файли**: `mobile/src/features/randomize/**` (нові),
`mobile/src/app/(tabs)/tournaments/[id]/randomize.tsx` (новий), `docs/MOBILE_APP.md`.
...
`globals.css`) і фонове фото трохи зумиться (`group-hover:scale-105`). `transform: scale()` не
змінює розмір layout-боксу, але Chromium враховує пофарбовані межі трансформованого елемента у
scrollable overflow предка — а `.split-panel` мав `overflow-y-auto` прямо на собі, тож зумлене фото
провокувало реальний вертикальний скролбар (підтверджено Playwright: `scrollHeight` 900→923px
рівно на зумленій панелі, і лише під час/після hover). Фікс — обгорнути `<Image>` в окремий
`absolute inset-0 overflow-hidden` wrapper, щоб зум фото обрізався там, а не долітав до
scrollable-overflow панелі.
...
За запитом користувача (скріншот навбару теніс-хабу). У списку `NAV_LINKS`
(`src/lib/site.ts`) «Турніри» стояло одразу після «Теніс», перед «Ціни» та «Тренери».
Перенесено так, щоб «Турніри» йшло безпосередньо перед «Матчі»: Теніс → Ціни → Тренери →
Турніри → Матчі → Статистика → Рейтинг → Гравці → Новини → Фото. `PADEL_NAV_LINKS` не
чіпали — там «Турніри» вже стоїть одразу перед «Матчі».

**Файли**: `src/lib/site.ts`. Тести `tests/components/nav.test.tsx` та
`tests/components/nav-links.test.tsx` (27 тестів) пройшли без змін.

## 2026-09-01 — Парний рейтинг більше не показує від'ємні числа

За запитом користувача (скріншот сторінки `/rating` — кілька гравців унизу парного рейтингу
мали від'ємні бали, що виглядає демотивуюче). Причина: `conservativeOrdinal`
(`src/lib/rating/openskill.ts`) рахує `mu - 3*sigma` *до* застосування власного +1500 зсуву
`displayRating` — тож новачок (mu=25, sigma=25/3) після штрафу невизначеності отримує ordinal≈0,
а не 25, і на екрані стартує з ~479, а не з ~1500, як в одиночному Glicko-2. Гравець із більшою
кількістю поразок, ніж перемог, після цього легко йде у мінус. Додано константу
`CONSERVATIVE_DISPLAY_OFFSET = 1000` — суто для відображення, повертає стартову позначку
новачка ближче до ~1500. Порядок місць, дельти рейтингу після матчів і сама математика
mu/sigma не змінюються (однакова константа для всіх скорочується і в різниці, і в сортуванні).
Торкається всіх місць, де показується парний OpenSkill-рейтинг: `/rating`, `/padel/rating`,
профіль гравця, графіки історії рейтингу.

**Файли**: `src/lib/rating/openskill.ts`. Повний прогін тестів (1758) пройшов без змін в інших
файлах.

## 2026-09-01 — Фікс: фільтри на сторінці гравця скидали прокрутку на початок сторінки

За запитом користувача (скріншот профілю гравця — після вибору фільтра за датою/суперником
сторінку викидало нагору, хоча фільтри розташовані нижче середини сторінки). `router.push` у
`OpponentFilter`/`TournamentFilter` та `next/link` у `PillFilterLink` (формат/рік пілюльки та
фільтр стат-карток перемог/поразок) за замовчуванням скролять на початок сторінки при
навігації — навіть коли міняється лише query-параметр на тій самій сторінці. Додано
`{ scroll: false }` / `scroll={false}` до всіх трьох, щоб позиція прокрутки зберігалась.

**Файли**: `src/components/opponent-filter.tsx`, `src/components/tournament-filter.tsx`,
`src/components/pill-filter.tsx`, `tests/components/opponent-filter.test.tsx`,
`tests/components/tournament-filter.test.tsx`.

## 2026-09-01 — Фікс: діаграма розподілу рейтингу ставала величезною при ~30+ гравцях

За запитом користувача (скріншот сторінки `/rating`, парний рейтинг — графік "Розподіл
рейтингу" займав майже весь екран). Причина — баг у `layoutStripPlot`
(`src/lib/rating-distribution.ts`): алгоритм порівнював кожну точку лише з *попередньою* по
порядку, а не з фактичним вмістом кожної "доріжки" (lane). Довгий ряд гравців із близькими
рейтингами (типово для парного — 34 гравці) через це піднімався на нову доріжку з кожною точкою
підряд, навіть коли конкретна пара точок узагалі не колізила — висота графіка росла лінійно від
довжини всього ряду, а не від локальної щільності. Переписано на справжній greedy-бджолиний рій:
точка йде в найнижчу доріжку, чий останній займанець із нею не колізить (`lastPctByLane`).
Висота тепер обмежена реальною локальною скупченістю, а не загальною кількістю гравців.

**Файли**: `src/lib/rating-distribution.ts`, `tests/lib/rating-distribution.test.ts`.
...
`docs/UX_AUDIT_FIXES.md`-раундів — знайдено 7 місць, не покритих попередніми заходами; усі
виправлено цим же заходом.

**Критично**
- `src/components/triple-split.tsx` — панель «Кава» на головній не мала повноекранного
  `<Link>`, який мають «Теніс»/«Падел»; єдиний вхід був через CTA-кнопку, що з `md:` ставала
  `opacity-0` до `:hover` — на тач-пристроях без миші (планшети, тач-ноутбуки, телефон у
  landscape ≥768px) секція була фактично недосяжна. Додано такий самий full-panel `<Link>`
  для Кави.
- Той самий файл — `overflow-hidden` → `overflow-y-auto` на панелі: на низькому viewport
  (landscape-телефон, `h-dvh` ~350-400px при `min-h-[460px]`) контент міг обрізатись без
  індикатора скролу.

**Помітно**
- Тач-таргети розширено до ~44px (рекомендований мінімум), там де раніше вони були 28-32px:
  `src/components/photo-lightbox.tsx` (стрілки/видалення/закриття в лайтбоксі),
  `src/components/theme-toggle.tsx`, `src/components/background-toggle.tsx` (обгортка `<label>`
  довкола `Switch`), `src/components/tie-card.tsx` (кнопка видалення зустрічі — невидима
  розширена тап-зона через `::after`, щоб не зламати вирівнювання із сусідньою кнопкою).
- `src/components/tournament-standings.tsx`, `src/app/leaderboard/page.tsx` — колонку «Геймів»
  сховано нижче `sm:` (`hidden sm:table-cell`), той самий патерн, що вже був у
  `admin/tournaments-table.tsx`.
- `src/components/ui/dropdown-menu.tsx` — додано `collisionPadding`, як у `select.tsx`, щоб
  мобільне бургер-меню (`nav.tsx`) не впиралось у край екрана/OS-хром.

**Дрібниці**
- `src/app/globals.css` — `overflow-x: clip` на `html`: кілька сторінок ламають `<main>`'ову
  max-width через `-mx-[50vw] w-screen` (`page.tsx`, `coffee/page.tsx`,
  `tennis/pricing`, `tennis/coaches`) — `100vw` може бути ширшим за `100%` там, де десктопний
  браузер резервує смугу скролу. `clip`, не `hidden` — `hidden` на предку ламає
  `position: sticky` в нащадках (sticky-колонки таблиць, нав).
- `src/components/horizontal-scroller.tsx` — стрілки прокрутки сховано на тач
  (`pointer-coarse:hidden`): свайп і так працює, а стрілки могли перекривати крайню картку на
  вузькому екрані.

Перевірено: `tsc --noEmit`, `eslint` на змінених файлах, `npm run build`.
...
прогалин (`src/lib/r2.ts`). `?? "photo"` замінено на `|| "photo"`: `.pop()` після
`String.prototype.split` для рядка ніколи не повертає `undefined` (навіть `"".split(...)` дає
`[""]`), тож `??` там ніколи не спрацьовував — вироджений вхід (порожній рядок, шлях, що
закінчується роздільником) давав порожній ключ замість задуманого фолбека `"photo"`. `||`
коректно ловить і цей випадок, бо `base` завжди рядок (ніколи `null`/`undefined`/`0`/`false`) —
жодного небажаного приведення типів. `tests/lib/r2.test.ts` оновлено під виправлену поведінку.
...
під `/coffee` чи `/padel` — включно з `/admin`, спільним для всіх доменів розділом. Admin
Падела чи Кави, відкривши адмін-панель, бачив повний тенісний рядок (Тенic, Турніри, Ціни,
Тренери...), що виглядало так, ніби його "закинуло" у секцію Тенісу.
- `src/components/nav-home-hide.tsx`: нова `isGenericPage(pathname)` (`/` або `/admin/*`) —
  `HideOnHome`/`ShowOnHomeIfAuthorized` тепер спираються на неї замість лише `pathname === "/"`.
  На `/admin/*` тенісний рядок посилань і мобільний "гамбургер" ховаються так само, як і на
  головній `/`, а натомість показується той самий окремий лінк "Адмін-панель" — свій AdminNav
  (Огляд/Гравці/Новини/…, вже коректно відфільтрований по доменах адміна) лишається єдиною
  навігацією в адмінці.
- `HideOnHubPages` (той самий файл) теж підпадає під `isGenericPage` для консистентності —
  тенісний фон-тогл більше не показується на `/admin`.
- Тести: `tests/components/nav.test.tsx` — новий describe-блок для `/admin` (і його підсторінок)
  з PADEL/COFFEE-адміном.
- Повний прогін: 167 файлів, 1543 тести — 0 регресій. `tsc --noEmit`/`eslint` чисті.
...
рядками для тих, хто ще не зіграв жодного матчу — але тільки коли не активний жоден фільтр
(`.filter((row) => row.matchesPlayed > 0 || !hasFilter)`); увімкнення будь-якого фільтра одразу
ховало нулі. Особливо помітно на щойно запущеному Паделі: усі гравці клубу з'являлись у таблиці
з суцільними нулями, бо жодного падел-матчу ще не зіграно.
- `src/app/leaderboard/page.tsx`, `src/app/padel/leaderboard/page.tsx`: прибрано `|| !hasFilter` —
  рядок показується лише за наявності хоча б одного зіграного матчу (у відповідному виді спорту),
  незалежно від фільтрів. Порожній стан без фільтра тепер каже "Ще ніхто не зіграв жодного
  матчу." замість "Ще немає жодного гравця." (гравці є, просто ще без матчів).
- `/rating` і `/padel/rating` цього не потребували — там рядки й так будуються лише з
  фактичних матчів (`getSinglesRatings`/`getPadelSinglesRatings` тощо), без злиття з повним
  списком гравців.
- Повний прогін: 167 файлів, 1532 тести — 0 регресій. `tsc --noEmit`/`eslint` чисті.

## 2026-08-14 — Нав: посилання назад на /tennis і /padel хаби

З будь-якої тенісної сторінки (наприклад `/tournaments`) не було способу повернутись на хаб
`/tennis` (той самий, що показує `SITE_NAME`/опис клубу/картки-розділи) інакше, ніж через
головну `/` (потрійний спліт) і повторний клік по тенісній панелі — логотип у `nav.tsx` веде на
`/`, а `NAV_LINKS` не містив запису на сам `/tennis`. Той самий пробіл був і в `PADEL_NAV_LINKS`
(жодне з чотирьох посилань не веде на сам `/padel`).
- `src/lib/site.ts`: додано `{ href: "/tennis", label: "Теніс", exact: true }` першим елементом
  `NAV_LINKS` і `{ href: "/padel", label: "Падел", exact: true }` першим елементом
  `PADEL_NAV_LINKS`.
- `src/components/nav-links.tsx`: `NavLink` отримав необов'язкове поле `exact` — без нього
  `pathname.startsWith(href)` позначав би "Теніс"/"Падел" активними на кожній під­сторінці
  своєї секції (той самий принцип, що вже застосований для `/admin` в `admin-nav.tsx`).
- Побічний факап, знайдений під час фінального прогону тестів для цієї зміни: попередній коміт
  (Падел-фото-паритет) додав імпорт `deletePhotoAction` у `tournament-gallery.tsx`, а
  `tests/components/tournament-gallery.test.tsx` цей імпорт не мокав — тест валився з
  `DATABASE_URL is not set`. Прогін перед тим комітом показав "2 failed" тестових файли, але
  перевірено вручну було лише відомий флейкі-тест (`create-tie-dialog.test.tsx`) — другий факт
  збою пропущено. Виправлено тут: доданий `vi.mock("@/lib/actions/photos", ...)` у цей тест.
- Повний прогін: 167 файлів, 1532 тести — 0 збоїв. `tsc --noEmit`/`eslint` чисті.
...
11 — тести — уплетено в кожен крок окремо, а не винесено сюди). Повна паритетність: групи,
сіяність, командна/зустрічна гра (MIXED), каскадне оновлення сітки плей-офф, усі три варіанти
рандомайзера (одиночний round-robin, groups12-плейофф, парний), Статистика й Рейтинг
(Glicko-2 одиночний + OpenSkill парний + бали Set Club), окрема адмін-навігація
(`/admin/padel/tournaments/*`, чітко відокремлена від тенісної), і власний публічний
навігаційний блок (поки видимий лише адмінам Падела/суперадмінам — реальні корти ще не
відкриті).
- Архітектура: повністю окремі `Padel*`-таблиці (не discriminator-колонка на тенісних), гравці/
  новини/фото лишились спільними на весь клуб без окремих Padel-версій.
- Перевикористання: усі чисті (без прив'язки до Prisma) файли — `bracket-advancement.ts`,
  `randomize-pairs.ts`, `groups12-playoff-bracket.ts`, увесь `rating/*` рушій, `head-to-head.ts`,
  `player-stats.ts`, схеми валідації, CSV-експортери — використані як є, без клонування.
  Презентаційні компоненти (`MatchSummary`, `TournamentPlayoffs`,
  `TournamentStandingsSection`, `TournamentTiesSection` тощо) теж перевикористані завдяки
  структурній (не номінальній) типізації Prisma — підтверджено `tsc` на кожному кроці.
- Підсумковий прогін: 164 тестових файли, 1507 тестів (було 1184 на початку епіку) — 0
  збоїв. Повний `tsc --noEmit` і `eslint .` по всьому репо — чисто (одне попередження в
  `coverage/block-navigation.js`, згенерованому артефакті, не пов'язане з цією роботою).
- Усі 10 комітів цього епіку лишаються локальними до явного підтвердження користувача перед
  пушем.
...
- `src/lib/queries/padel-tournaments.ts`, `padel-matches.ts`, `padel-tournament-teams.ts` —
  прямі порти `queries/tournaments.ts`/`matches.ts`/`tournament-teams.ts` на нові Padel-моделі.
  Функції "Рік у SET.club" (сезонні лічильники для шер-картки) свідомо не портовані — це окрема
  фіча поза межами Турніри/Матчі/Статистика/Рейтинг.
- Тести: `tests/lib/queries/padel-tournaments.test.ts`, `padel-matches.test.ts` (18 тестів,
  дзеркалять покриття тенісних аналогів).
...
посилання (навмисно, секція під розробкою). Тепер авторизований глядач (суперадмін або Admin з
доменом PADEL) отримує реальну точку входу прямо на головній, без зміни вигляду для решти.
- `src/app/page.tsx` — став `async`, читає `auth()`/`getAdminScope`, рахує `padelAuthorized`.
- `src/components/triple-split.tsx` — `TripleSplit`/`SplitPanel` приймають `padelAuthorized`;
  коли true, панель "Падел" отримує клікабельний оверлей (як у Теніс) + кнопку "Переглянути
  (адмін)" замість вимкненого "Незабаром". Бейдж "Coming Soon" і затемнення лишаються для всіх —
  контенту, вартого приховування, там ще немає, це просто зручніший вхід для адмінів.
...
`MenuSection`/`MenuItem`, 12 фото напоїв завантажено в R2 (`menu/...`) — одноразовим скриптом
(`tsx` + `@prisma/adapter-neon` + `@aws-sdk/client-s3` напряму, бо `src/lib/r2.ts` захищений
`server-only` і не імпортується поза Next-білдом; скрипт запущено разово й видалено, не
закомічено — повторний запуск задублював би рядки, бо на назву секції/напою нема unique-обмеження).

`src/app/coffee/page.tsx` тепер `async` серверний компонент: `getActiveMenuSections()` замість
`DRINK_GROUPS`/`SPECIAL_MENU`/`MATCHA_MENU`, фото — `publicPhotoUrl(item.photoKey)` замість
локальних `/coffee/drinks/*.png` шляхів. Розбиття на LIST/CARDS секції за `section.layout`; зелений
"matcha"-акцент картки визначається по `section.name.toLowerCase().includes("matcha")` (не окреме
поле в БД — навмисно мінімально, колір це деталь оформлення двох конкретних секцій, а не щось, що
змінюється часто). Видалено `public/coffee/drinks/*.png` (12 файлів) — тепер непотрібні дублікати
того, що вже в R2; три оригінальні композитні JPG (`drinks-menu.jpg`, `special-menu.jpg`,
`matcha-menu.jpg`) лишились як довідковий оригінал.

Перевірено: скриншот `/coffee` після переключення пікселе-в-піксель співпав з версією на хардкоді;
`/admin/menu` показує всі 6 реальних секцій. Повний прогін `vitest` (1102 тести) — зелений.

## 2026-08-13 — Меню кав'ярні в БД: MenuSection/MenuItem + повна адмінка

Досі меню на `/coffee` було хардкод-масивом у `page.tsx` — Coffee-адмін не міг нічого редагувати
без PR. Обговорили з користувачем структуру полів перед тим, як писати код, і зійшлись на:
`MenuSection` (назва, опційний tagline, `layout: LIST | CARDS`, `sortOrder`,
`active`) → має багато `MenuItem` (назва, ціна в грн, опційний опис, опційне фото через R2,
`sortOrder`, `active`). `layout` — це і є "категорія" одним фіксованим enum, а сама секція
("Кава", "Special Menu"...) — вільний текст, який задає адмін; тому "Матча" більше не дублюється
структурно (звичайний пункт прайсу — просто MenuItem у LIST-секції, тематична добірка — окрема
CARDS-секція).

Зроблено (поки БЕЗ міграції реального контенту з /coffee — публічна сторінка досі на старому
хардкоді, це наступний крок):
- Схема: `prisma/schema.prisma` (`MenuLayout`, `MenuSection`, `MenuItem`) — застосовано міграцію
  `add_menu_section_item` до бойової Neon-БД (чисто адитивна, підтверджено з користувачем окремо,
  бо dev і прод — одна й та сама база).
- `src/lib/validation/menu.ts`, `src/lib/queries/menu.ts`, `src/lib/actions/menu.ts` (CRUD +
  `toggle*ActiveAction` для показати/приховати без видалення) — за тим самим патерном, що
  `players.ts`/`news.ts`.
- Фото напоїв — той самий "браузер → R2 напряму" флоу, що в новин: `MenuPhotoField`
  (`src/components/admin/menu-photo-field.tsx`) + `/api/menu/photo-presign`
  (гейт `isDomainAdmin("COFFEE")`, ключі `menu/...`).
- Адмінка: `/admin/menu` (гейт `isDomainAdmin("COFFEE")`, редірект на `/admin` інакше) — список
  секцій, у кожній її напої, діалоги створення/редагування (`MenuSectionDialog`/`MenuItemDialog`),
  кнопки видалення з підтвердженням і показати/приховати (`MenuToggleActiveButton`). Додано в
  `admin/page.tsx` (SECTIONS) і `admin-nav.tsx` (таб "Меню", `requiresDomain: "COFFEE"`).
- Audit log: 10 нових дій (`menu.section.*`, `menu.item.*`) у `audit-actions.ts`.
- Перевірено вручну через `/api/test-login` (ADMIN+COFFEE): створення секції й напою, редагування,
  показати/приховати, видалення — усе на реальній БД, дані прибрано після тесту.

Сортування — просте числове поле `sortOrder`, яке адмін заповнює вручну (лишаючи проміжки типу
10/20/30), без drag-and-drop — свідомий мінімальний варіант, у коді нема прикладу reorder-UI, який
можна було б повторити без окремої розробки.

## 2026-08-13 — "Адмін-панель" одразу на головній для будь-якого адміна

`HideOnHome` ховає весь Tennis-нав на `/` (триплспліт-хаб не має власних лінків), включно з
"Адмін-панель" — суперадмін чи будь-який доменний адмін мусив спершу перейти на `/tennis`, щоб
дістатись адмінки. Новий `ShowOnHomeIfAuthorized` (`nav-home-hide.tsx`) навмисно ламає цей
патерн: показує окреме посилання "Адмін-панель" одразу біля лого на `/`, коли `hasAdminAccess`
(той самий прапорець, що й скрізь — суперадмін або адмін з хоча б одним доменом). Тест
`nav.test.tsx` розділено на два: адмін-панель тепер видима на головній для суперадміна, і досі
відсутня для звичайного MEMBER.

## 2026-08-13 — Фікс: "Вийти" налазив на бейдж "Суперадмін" у Nav

Не пов'язано з /padel-роботою вище — користувач спіймав у реальному браузері (superadmin-сесія,
широкий десктоп) header, що рвався на два рядки: правий кластер (theme-тумблер + bg-фото-тумблер +
avatar/ім'я + бейдж "Суперадмін" + "Вийти") не влазив у виділений йому простір при `max-w-6xl`
(1152px) і `flex-wrap` мовчки переносив саме "Вийти" на другий рядок впритул під бейджем.
Виміряв через Playwright: лівий кластер (лого + до 10 nowrap nav-посилань) — стабільно ~725px,
правому кластеру реально треба ~420-450px — разом це вже перевищує 1152px, запасу не було
взагалі, навіть з коротким тестовим ім'ям на 1920px. Замінив `max-w-6xl` → `max-w-7xl` у
`src/components/nav.tsx` (коментар над рядком оновлено з цифрами замірів). Перевірено вручну
через `/api/test-login` (SUPERADMIN) на 1280/1366/1440/1536/1680/1920px — усюди тепер один рядок
без горизонтального скролу. Не чіпав вужчий "lg, але ще не xl" діапазон (1024-1149px, де бейджа
й так не видно) — там рядок все ще іноді рветься, але це вже не той конкретний баг, який зловив
користувач, і трапляється рідше на практиці (вузький desktop/tablet, не типовий десктопний монітор).

## 2026-08-13 — /padel: третій хаб, доступ лише Суперадміну/Падел-адміну

За тією ж моделлю, що й `/coffee` (запис нижче), плюс обмеження доступу: Падел ще не відкрито (на
головній триплспліт-панель Падел навмисно без `<Link>`, див. `triple-split.tsx`), тож поки що
пункт "Падел" у nav і власний тумблер фото-фону бачить лише `isSuperAdmin || domains.includes
("PADEL")` — для всіх інших `padelLinks` порожній масив, і на `/padel` нав не показує взагалі
нічого. Сама сторінка `/padel` (`src/app/padel/page.tsx`) при цьому не 403-ить і не редіректить —
хто завгодно, хто вгадає URL напряму, побачить той самий дружній плейсхолдер "У будівництві", що й
привілейований відвідувач: реального контенту, який справді треба ховати, там ще немає.

Технічно:
- `PADEL_NAV_LINKS` (`src/lib/site.ts`), `useSectionLinks()` розширено на 3 варіанти
  (`src/components/nav-links.tsx`).
- `BackgroundToggle` (`src/components/background-toggle.tsx`) перероблено з жорстко зашитих
  `STORAGE_KEY`/`HTML_CLASS` на пропси — тепер один компонент обслуговує і тенісний тумблер
  (`setclub:bg-photo`/`bg-photo`), і новий падел-тумблер (`setclub:bg-photo-padel`/`bg-photo-padel`)
  незалежно один від одного.
- `HideOnHomeOrCoffee` → `HideOnHubPages` (тепер приховує тенісний тумблер і на `/padel` теж) +
  новий `ShowOnPadelIfAuthorized` (показує падел-тумблер лише на `/padel` і лише за
  `hasPadelAdminAccess`, який рахується один раз серверно в `nav.tsx`).
- `CoffeeBackgroundPhotoGuard` → `SectionRouteGuard` (перейменовано,
  `src/components/section-route-guard.tsx`) — тепер ставить обидва класи (`coffee-route`,
  `padel-route`) на `<html>`, а не тільки coffee.
- `globals.css`: тенісне фото тепер `:not(.coffee-route):not(.padel-route)`; нове правило
  `html.bg-photo-padel.padel-route body` для падел-фото (застосовується лише коли toggle
  увімкнено І користувач фактично на `/padel` — без окремого override-правила, на відміну від
  coffee, бо умова вже в самому селекторі).
- `layout.tsx`: другий anti-flash `<Script>` для `setclub:bg-photo-padel`, щоб не було спалаху
  неправильного фону до гідратації.

**Фото для падел-фону:** покладіть файл як `public/padel-court-bg.jpg` (аналогічно
`public/court-bg.jpg` для тенісу) — підключено в `globals.css`, більше нічого міняти не треба.

Перевірено вручну через `/api/test-login` (SUPERADMIN, ADMIN+TENNIS, ADMIN+PADEL, анонім) — у всіх
чотирьох випадках nav і обидва тумблери поводяться як очікується. Тести:
`tests/components/nav-links.test.tsx` (+2 нові на перемикання на /padel, +1 на порожній набір),
`tests/components/background-toggle.test.tsx` (+1 на незалежність двох інстансів). Повний прогін
vitest — 1101 тест, зелений.

## 2026-08-13 — /coffee як окремий хаб: власна навігація, без тенісного фону

`/coffee` показував весь тенісний nav (Турніри/Ціни/Тренери/Матчі/…) і фото-корту як фон, якщо
користувач раніше вмикав цей тумблер деінде (`html.bg-photo` — глобальний прапорець у
localStorage, не прив'язаний до розділу). Тепер `/coffee` — окремий хаб на кшталт `/`:

- Nav-посилання: `NAV_LINKS` (тенісні) замінено на `COFFEE_NAV_LINKS` — лише один пункт "Меню" —
  коли `pathname.startsWith("/coffee")`. Свіжий `ADMIN_NAV_LINK`/`COFFEE_NAV_LINKS` у
  `src/lib/site.ts`, вибір набору — у `useSectionLinks()` (`src/components/nav-links.tsx`,
  усередині вже клієнтських `NavLinksInline`/`NavLinksDropdownItems`, тож не знадобився жоден новий
  проброс pathname з серверного `nav.tsx`).
- "Адмін-панель" на `/coffee` показується лише за `isSuperAdmin || domains.includes("COFFEE")`
  (не "будь-який домен", як у дефолтному наборі) — рахується поруч у `nav.tsx`.
- Тумблер фото-фону (`BackgroundToggle`) прибрано на `/coffee` через новий `HideOnHomeOrCoffee`
  (`nav-home-hide.tsx`, сестра для вже наявного `HideOnHome`).
- Сам фон примусово вимкнено на `/coffee`, навіть якщо `bg-photo` вже увімкнено в localStorage:
  новий клієнтський `CoffeeBackgroundPhotoGuard` (`src/components/coffee-bg-photo-guard.tsx`,
  змонтований у `layout.tsx`) ставить/знімає клас `coffee-route` на `<html>` через `usePathname()`,
  а `globals.css` (`html.bg-photo.coffee-route body { background-image: none; }`) глушить лише
  зображення, не займаючи сам localStorage-прапорець — при переході назад на `/tennis` фото
  з'являється знову без додаткових дій користувача.

Тести: `tests/components/nav-links.test.tsx` оновлено під нову сигнатуру пропів
(`defaultLinks`/`coffeeLinks` замість одного `links`) + два нові тести на перемикання набору
посилань на `/coffee`. Повний прогін `vitest` (1097 тестів) — зелений.

## 2026-08-13 — Автоматичне вирізання фото напоїв замість ручного підбору меж

Ручний підбір меж лінійкою-накладкою (попередній запис нижче) все одно лишав огріхи — користувач
зловив обрізаний правий край "Кокосова матча" (ручна оцінка x-меж по зменшеному прев'ю була
неточна так само, як раніше з Y). Замість продовжувати вгадувати пікселі вручну, переписав
`public/coffee/drinks/*.png` через повністю автоматичне вирізання: для кожного з 6 рядків обох
композитних дошок беру широку смугу на всю ширину (без ручних x/y меж), роблю той самий
flood-fill-у-прозорість від країв смуги, тоді шукаю зв'язні компоненти непрозорих пікселів
(4-зв'язність, окремо в лівій і правій половині смуги) і залишаю найбільшу компоненту в кожній
половині — це і є сам напій, а не випадковий текст поруч (текст завжди дрібніший за компоненту
келиха). Bounding box найбільшої компоненти (+6px запасу) стає межами кропу — жодних ручних
координат. Результат: усі 12 фото тепер без винятку показують напій цілком з усіх чотирьох боків,
перевірено візуально по кожному. `MenuCard` (`src/app/coffee/page.tsx`) не змінювався — контейнер
уже був на `object-contain` в `aspect-[4/5]`, тож різні пропорції нових кропів вписуються без змін
у розмітці.

## 2026-08-13 — Виправлено обрізані знизу фото напоїв на /coffee

Перший прохід кропу (попередній запис нижче) визначав межі кожного напою на око по зменшеній
превʼю-картинці — і систематично недооцінював, де в оригіналі (1179×2096) насправді закінчується
келих: у більшості випадків низ келиха/чашки обрізався ще до дна. Перекроїв усі 12 позицій наново,
цього разу вимірюючи точні межі лінійкою-накладкою (сітка з підписаними Y-координатами,
композитована поверх zoom-кропу через `sharp`) окремо для кожного з 6 рядків обох дошок — знайшов
справжні top/bottom для кожного келиха й окремо перевірив, де починається підпис назви під фото,
щоб крок не захопив шматок тексту (інакше flood-fill з `[[2026-08-13 — Фото кожного напою на
/coffee|попереднього кроку]]` лишає його як "летючий" непрозорий острівець, бо текст не зв'язаний
з фоном). Найтугіші випадки — "Тропік брю" й "Імбирне айс-лате" (низ келиха практично впритул до
підпису, ~5 px запасу). Файли не змінились (ті самі `public/coffee/drinks/*.png`), тільки їх вміст.

Клуб не має окремих фото напоїв — лише 3 вже готові Canva-дошки (`special-menu.jpg`,
`matcha-menu.jpg`), де кілька напоїв зображені разом з текстом в одному растрі. Замість того щоб
йти без фото в картках `MenuCard`, вирізав усі 12 фото напоїв (Special Menu + Matcha Menu) з цих
композитних дошок через `sharp` (уже був у `node_modules` — next/image optimization) прямо в
одноразовому Node-скрипті: підбирав `left/top/width/height` для кожної позиції ітеративно,
звіряючи проміжні кропи візуально, доки скло не влазило в кадр без сусідніх написів. Результат —
`public/coffee/drinks/*.jpg` (12 файлів), підключені в `MenuCard` над назвою напою. Оригінальні
3 композитні дошки лишились у `public/coffee/` як джерело.

## 2026-08-13 — Повне меню кав'ярні на /coffee

Замінено заглушку "Затишна кав'ярня клубу вже готується" на повне меню, транскрибоване з трьох
фото клубу (`public/coffee/drinks-menu.jpg`, `special-menu.jpg`, `matcha-menu.jpg`): базові напої
(кава/чай/матча/інше — 20 позицій), Special Menu (6 фірмових напоїв з описом) і Matcha Menu (6
позицій). Дизайн свідомо відрізняється від темних `/tennis/*` — тепла кремова палітра
(`#f7ede1`/`#2b241d`/`#5b7a5e` sage-зелений), два нових Google-шрифти через `next/font/google`
(Playfair Display для заголовків, PT Serif для тіла) підключені локально в
`src/app/coffee/page.tsx`, не в кореневому `layout.tsx` — щоб не тягнути їх на весь сайт заради
однієї сторінки. Самі композитні фото-джерела на сторінці не показані (це растр тексту, який уже
повністю продубльовано як реальний текст) — лежать у `public/coffee/` як довідковий оригінал; фото
окремих напоїв, вирізаних з них, додано окремим кроком нижче.

Замінено плейсхолдер `+380 XX XXX XX XX` / `tel:+380000000000` на реальний номер клубу
`+380 93 313 27 05` у трьох місцях, де він дублювався окремими константами (`/tennis/pricing`,
`/tennis/coaches`, футер головної). Заодно винесено в один спільний `SITE_PHONE`/`SITE_PHONE_TEL`
у `src/lib/site.ts` (поруч з уже існуючими `SITE_NAME`/`NAV_LINKS`) замість трьох копій одного й
того самого плейсхолдера — щоб наступна зміна номера була в одному місці. Файли:
`src/lib/site.ts`, `src/app/tennis/pricing/page.tsx`, `src/app/tennis/coaches/page.tsx`,
`src/components/home-footer.tsx`.

## 2026-08-13 — Реальні ціни на /tennis/pricing

Замінено плейсхолдер "XXXX ₴" (оренда корту + індивідуальне тренування) на реальний прайс клубу:
оренда корту (400 ₴/год), прокат інвентаря — 2 ракетки + 3 м'ячики (200 ₴/год), корзина м'ячів
(150 ₴/год), тенісна пушка + корзина м'ячів (400 ₴/год). Дані та фото прайсу — від користувача
(`public/pricing/prices.jpg`), короткий опис під кожною позицією написаний на основі фото. Сітка
перебудована на двоколонковий layout (фото прайсу зліва, картки цін справа) замість одноколонкового
плейсхолдера. Файли: `src/app/tennis/pricing/page.tsx`, `public/pricing/prices.jpg`.

## 2026-08-13 — Біо всіх трьох тренерів на /tennis/coaches

Додано опційне поле `bio` (масив рядків-буллетів) до `COACHES` у `src/app/tennis/coaches/page.tsx`
— рендериться списком під іменем, якщо непорожнє. Дані від користувача (скріншоти) для всіх
трьох тренерів: Ліна Чаура (досвід/титули/мотивація, з правкою "18 років" → "більше 20 років"),
Денис Іоганов (титули/досвід/підхід, з правкою "15-річний досвід" → "17-річний"), Ірина Кулєш
(досвід/титули/мотивація, з правкою "майже 15 років" → "17 років").

## 2026-08-13 — Нове фото Дениса Іоганова на /tennis/coaches

Замінено `public/coaches/denys-iohanov.jpg` (оригінальний промо-банер із вбудованим написом
"SET.club НАШІ ТРЕНЕРИ", що дублював заголовок сторінки) на чисте фото `denys-iohanov.png` —
тепер картка виглядає узгоджено з рештою тренерів (лише маленький логотип у кутку). Файли:
`public/coaches/denys-iohanov.png` (нове), `public/coaches/denys-iohanov.jpg` (видалено),
`src/app/tennis/coaches/page.tsx`.

## 2026-08-12 — Реальні фото Кава/Теніс на головному тріпл-спліті

Замінено Unsplash-плейсхолдери панелей "Кава" і "Теніс" на реальні фото клубу — локальні файли
`public/split/coffee.jpg` (кав'ярня SET.club) і `public/split/tennis.jpg` (корт, м'ячі Babolat).
Падел лишився на Unsplash-плейсхолдері — фото будівництва корту ще немає. Файли:
`src/components/triple-split.tsx`.

## 2026-08-12 — Реальні тренери на /tennis/coaches

Замінено плейсхолдер-масив `COACHES` (4 вигаданих імені, фото з Unsplash) на трьох реальних
тренерів клубу — Іоганов Денис, Кулєш Ірина, Чаура Ліна. Фото — локальні файли в
`public/coaches/` (`denys-iohanov.jpg`, `iryna-kulesh.jpg`, `lina-chaura.jpg`), а не R2 чи
Unsplash: сторінка й далі статична (без БД/адмінки), тож локальні файли — найпростіший варіант.
Поля "досвід"/"спеціалізація" прибрано з карток — реальних даних поки нема, а фейкові текстові
плейсхолдери гірші за їх відсутність. Сітка звужена з 4 до 3 колонок (`sm:grid-cols-3`) під нову
кількість тренерів. `next.config.ts`: коментар при `images.unsplash.com` уточнено — цей домен
тепер потрібен лише для фону тріпл-спліт головної (`triple-split.tsx`), не для тренерів.
Файли: `src/app/tennis/coaches/page.tsx`, `next.config.ts`, `public/coaches/*`.

Якщо згодом з'явиться потреба редагувати тренерів через адмінку (як `Player`), доведеться додати
модель `Coach` у Prisma, серверні екшени й R2-аплоад фото — поки цього не робили, бо запит був
саме "додати реальних тренерів", а не побудувати CRUD.

## 2026-08-12 — Редизайн головної #2: Тріпл-Спліт на 100vh (Кава/Теніс/Падел) + нові сторінки тенісу

Гілка `redesign/triple-split-v2` (окремо від `redesign/split-screen-v1`). Головну (`src/app/page.tsx`)
замінено на темний тріпл-спліт на весь екран: `src/components/triple-split.tsx` (Кава/Теніс/Падел,
hover-розширення 33%→50%/25%/25% через `:has()` в `globals.css`, чистий CSS без JS) і новий
брендований футер `src/components/home-footer.tsx`. Поточний вміст головної переїхав на `/tennis`;
додано `/tennis/pricing` (плейсхолдер ціни), `/tennis/coaches` (галерея тренерів, запис лише по
телефону) і мінімальну заглушку `/coffee`. `next.config.ts` дозволяє `images.unsplash.com` для
плейсхолдер-фото. Деталі дизайну, знайдений під час верстки stacking-context нюанс і зауваження
щодо флакі click-тестів headless Chromium на `position: absolute` лінках — у `docs/HOMEPAGE.md`.

Ця гілка змерджила `master` (нижче) — після цього моменту адмінка на редизайн-гілці теж має
трирівневу модель прав (Суперадмін/Адмін+домен/Учасник), не стару пласку.

## 2026-08-12 — Ревізія адмін-прав: реальний баг + дублювання + e2e-покриття

Пройшовся по всій щойно зробленій фічі домен-прав у пошуках зайвого/неперевіреного:
- **Реальний баг:** пониження `ADMIN → MEMBER` не чистило записи `UserAdminDomain` — вони просто
  ставали неактивними, поки роль `MEMBER`, але мовчки "оживали", якщо цю людину колись знову
  підвищать до `ADMIN` з абсолютно іншого приводу. `updateUserRoleAction` тепер явно видаляє їх
  при пониженні до `MEMBER` (в одній транзакції з оновленням ролі).
- **Дублювання:** логіка "хто суперадмін / які домени рахувати" була по-різному написана в трьох
  місцях (`admin/layout.tsx`, `admin/page.tsx`, `nav.tsx`) — винесено в один хелпер
  `getAdminScope()` (`src/lib/permissions.ts`), усі три місця тепер використовують його.
- **Тестова прогалина:** нову доменну модель (SUPERADMIN / ADMIN+домен / ADMIN без домену)
  перевіряв тільки я вручну через тимчасові скрипти — тепер це три постійні тести в
  `e2e/admin-flows.spec.ts`. Для цього `/api/test-login` навчився приймати опційні `role`/`domains`
  у тілі запиту (за замовчуванням — як і раніше, SUPERADMIN).
- Попутно виявив (не мій регрес, не займав): "admin can create a player" у тому ж e2e-файлі
  тепер падає через пагінацію — реальна БД доросла до 35+ гравців, і новостворений не потрапляє
  на першу сторінку списку.

Деталі — в оновленому `docs/ADMIN_DOMAINS.md`.

## 2026-08-12 — Трирівнева роль: Суперадмін / Адмін (+ домен) / Учасник

Уточнення щойно зробленої фічі домен-прав: замість "ADMIN одразу = суперадмін, домени висять на
MEMBER" — тепер `Role` має три значення (`SUPERADMIN`/`ADMIN`/`MEMBER`), і домени
(Кава/Теніс/Падел) мають значення лише для `ADMIN` (спершу підвищуєте учасника до `ADMIN`, потім
призначаєте йому домен). Дата-міграція перенесла попередніх `ADMIN` у `SUPERADMIN`, а `MEMBER` із
вже призначеними доменами (Марго, Денис) — у `ADMIN`. Оновлено `permissions.ts`
(`isAdmin`/`requireAdmin` тепер саме про `SUPERADMIN`), усі місця, що раніше звіряли
`role === "ADMIN"` як ознаку суперадміна (`admin/layout.tsx`, `admin/page.tsx`,
`gallery/[id]/page.tsx`, `components/nav.tsx` — останній ще й отримав виправлення: пункт
"Адмін-панель" у головному меню раніше показувався тільки суперадміну, тепер і домен-адміну теж,
інакше той не мав як дістатись `/admin`). `UserRoleSelect` — трирівневий, підтвердження лише на
підвищення до `SUPERADMIN`. `UserDomainsEditor` тепер показується тільки для `ADMIN`-рядків
(для `MEMBER` — прочерк, для `SUPERADMIN` — "Усі розділи"). Деталі й хід ручної перевірки — в
`docs/ADMIN_DOMAINS.md`.

## 2026-08-12 — Домен-скоуповані адмін-права: суперадмін + Кава/Теніс/Падел

Нова модель прав доступу для адмінки: окрім суперадміна (`Role.ADMIN`, повний доступ всюди,
без змін) з'явились скоуповані ролі "Адмін тенісу"/"Адмін кав'ярні"/"Адмін паделу"
(`AdminDomain` enum + `UserAdminDomain` join-таблиця, юзер може мати будь-яку комбінацію).
Увесь наявний контент адмінки — тенісний, тож усі server actions
(`src/lib/actions/{matches,news,photos,players,randomize-*,teams,ties,tournaments}.ts`),
CSV-експорти, presign-роути фото і сторінки `/admin/tournaments*`, `/admin/players`,
`/admin/news` тепер перевіряють `requireDomainAdmin("TENNIS")`/`isDomainAdmin("TENNIS")` замість
`requireAdmin()`/`isAdmin()`. `/admin/users` і `/admin/audit` лишились суто суперадмінськими.
`AdminNav` і `/admin`-огляд фільтрують посилання за доменами; нова колонка "Адмін-розділи" на
`/admin/users` (`UserDomainsEditor`) дозволяє призначати домени. Кава і Падел поки без власних
адмін-сторінок (немає контенту) — лише права напоготові. Деталі, знайдені під час верстки нюанси
(окремий page-level гейт для тенісних сторінок, бо спільний layout-гейт пропускає БУДЬ-якого
домен-адміна) і хід ручної перевірки — в `docs/ADMIN_DOMAINS.md`.

## 2026-08-12 — Захист головного адміністратора від пониження

Користувач попросив: email `sdalin777@gmail.com` (єдиний запис в `ADMIN_EMAILS`, звідки й отримує
роль ADMIN при першому вході — див. `createUser` у `src/lib/auth.ts`) не повинен піддаватись
пониженню жодним іншим адміном через `/admin/users`.

Замість нової ролі в enum (міграція БД заради єдиного користувача була б надлишковою) —
`ADMIN_EMAILS` тепер трактується як список "постійних" адмінів: спільний хелпер
`src/lib/admin-emails.ts` (`getProtectedAdminEmails`/`isProtectedAdminEmail`) використовується і
для бутстрапу ролі при вході (`auth.ts`), і як захист у `updateUserRoleAction`
(`src/lib/actions/users.ts`) — будь-яка спроба змінити роль користувача з email зі списку кидає
помилку, незалежно від того, хто саме адмінить. На сторінці `/admin/users` такий рядок тепер
позначений міткою "(головний адмін)", а сам селект ролі задизейблений з поясненням у тултіпі
(`UserRoleSelect` отримав новий `disabledReason`).

Якщо в майбутньому знадобиться захистити ще когось — досить додати email через кому в
`ADMIN_EMAILS`.

## 2026-08-11 — Фікс: турнір з кінця списку відкривався не з початку (тільки в PWA)

Користувач повідомив: у встановленому PWA-застосунку (standalone-вікно без адресного рядка) сторінка
турніру, відкритого зі скролу в самий кінець списку "Турніри", відкривалась не з верху (де таблиці),
а десь посередині. У звичайній вкладці браузера (dev і прод) відтворити не вдалось попри чимало
спроб (програмний скрол + клік, "флінг" колесом без паузи, перехід назад-вперед, локальна
production-збірка, сам живий `set-club.vercel.app`) - завжди коректний scrollY=0.

Причина - у власній "розумній" логіці скролу App Router: при переході Next сам вирішує, чи скролити
нагору, перевіряючи, чи новий контент вже "у зоні видимості" за СТАРОЮ позицією скролу - перевірка,
що залежить від точної висоти вʼюпорту. У PWA-вікні (без панелі вкладок/адресного рядка) ця висота
інша, ніж у звичайній вкладці, і збіг умов, вочевидь, трапляється саме там.

Замість боротьби з цією евристикою - явний форс: новий `ScrollToTopOnMount`
(`src/components/scroll-to-top-on-mount.tsx`) викликає `window.scrollTo(0, 0)` в `useEffect` при
кожній зміні `resetKey` (id турніру), і підключений на `/tournaments/[id]`. 4 нових тести. Той самий
клас багу міг зачепити й інші сторінки, відкриті зі скрольованих списків (новини, гравці, фото) -
якщо десь ще проявиться, той самий компонент можна підключити туди ж.

Ще одне уточнення щойно зробленого doubles-фікса: замість того, щоб ранжувати кожну групу окремо
(і показувати "місце 4" з однієї групи вище за "місце 5" з іншої, навіть коли друга мала кращий
результат), лишок тепер порівнюється УСІМА РАЗОМ — тими самими критеріями, що й скрізь у таблицях
(перемоги → % перемог → особиста зустріч → різниця геймів → ім'я). Якщо особистої зустрічі між
двома командами/гравцями немає (а це майже завжди так для команд з різних груп), порівняння просто
переходить до наступного критерію, а не блокує розстановку місць — той самий принцип, за яким
`compareHeadToHead` уже давно повертає 0 для непройденої пари. Через це зникла й вимога "власний
круговий групи має бути повністю відіграний" — тепер місце отримує кожен, хто лишився, завжди.

Стосується і пар (`buildGeneralPlacedTableForTeams`), і одиночних/змішаних турнірів
(`buildGeneralPlacedTable` — той самий підхід до кастомних груп типу "Група за 7-10 місце" та
до звичайного лишку без явного поділу). Для одиночних/змішаних ранжування тепер рахується прямо
з матчів (`buildScopedSinglesRows` з новим прапорцем `includeAllRounds`), а не з окремо
агрегованої статистики турніру — так плейофф-реванш між двома лишковими гравцями теж рахується.

5 тестів переписано під нову поведінку (порівняння між групами через різницю геймів; одна лишкова
команда без суперників для порівняння всеодно отримує місце; лишкові гравці, що ніколи не грали
один одного, ранжуються за іменем). Повний набір: 1048/1048. Живою перевіркою на тому ж реальному
турнірі (`21.06.2025`, 2 групи по 3 пари) підтверджено: місця 3-4 і 5-6 тепер справді порівнюються
між групами за різницею геймів, а не просто йдуть блоками "спершу Група A, потім Група B".

## 2026-08-11 — Підсумкова таблиця для пар: місця й парам, які не потрапили в плей-офф

Продовження щойно зробленої doubles-`placedTable`: пари поза плей-офф лишались із "—" замість
місця, навіть коли їхня власна група вже повністю відіграна. `buildGeneralPlacedTableForTeams`
тепер після декисивних плейофф-місць проходить кожну вбудовану й кастомну групу окремо — якщо
конкретно ЦІЄЇ групи внутрішній кругови ще не заповнені пари ранжуються за її вже готовим
результатом і отримують наступний блок місць підряд. Свідомо не одним спільним "усі, хто лишився"
колом (як спершу зробив) — дві пари з РІЗНИХ груп майже ніколи не грали одна одну, тож такий
загальний круговий ніколи не був би "повним"; обробка групами по одній усуває це. Останній
прохід — і досі "усі, хто лишився" загалом, для випадку без вираженого поділу на групи.

2 нові тести (правильний порядок місць по групах; лишається без місця, коли своя ж група не
дограна). Повний набір: 1048/1048. Живою перевіркою на реальному турнірі (`21.06.2025`, 2 групи по
3 пари) підтверджено: усі 6 місць тепер заповнені — 1-2 з фіналу, 3-4 за Групою A, 5-6 за Групою B.

Раніше `getTournamentStandingsRows` (`src/lib/tournament-standings.ts`) рахувала "Підсумкову
таблицю" (`placedTable`) лише для SINGLES/MIXED — DOUBLES-гілка виходила з функції раніше й ніколи
не викликала жодного placedTable-білдера, навіть коли турнір мав реальний плей-офф (Фінал/За 3
місце тощо). Додано `buildGeneralPlacedTableForTeams` — парний аналог уже наявної
`buildGeneralPlacedTable`, що резолвить місця пар через той самий `resolveDecisivePlacements`
(він і так дженерик над рядковими ключами, тож команда-ключ "playerId1+playerId2" підходить без
жодної зміни самої функції). На відміну від одиночної версії, без фолбеку для пар, яких плей-офф
не охопив (лишаються "—") — вужче свідомо, можна розширити пізніше, якщо знадобиться.

Заразом виправлено супутню прогалину: таблиця "N учасників" (яка показується ДО Підсумкової)
для DOUBLES рахувала пари з урахуванням плей-офф-матчів теж (та сама вада, що вже була пофіксена
для SINGLES/MIXED раніше, але не для DOUBLES) — тепер вона, як і для одиночних, рахує лише
матчі групового етапу, а Підсумкова таблиця — весь турнір разом із плей-офф.

UI-змін не було: `TournamentStandingsSection`/`TournamentStandings` уже рендерять `placedTable`
дженерик, незалежно від того, гравці це чи пари. 4 нові тести в `tests/lib/tournament-standings.test.ts`.
Живою перевіркою на реальному парному турнірі з плей-офф (`27.06.2026`) підтверджено: Підсумкова
таблиця з'явилась, місця 1-4 коректні (переможець фіналу — 1-е місце), а таблиця ДО плей-офф
показує лише групові матчі.

## 2026-08-11 — "Завантажити ще" тепер довантажує й сам, при скролі

`LoadMore` (`src/components/load-more.tsx`) став клієнтським компонентом: `IntersectionObserver`
стежить за самою кнопкою-сентинелом і викликає ту саму навігацію (`router.push(href, {scroll:
false})`), щойно вона наближається до вʼюпорту (`rootMargin: 400px`) — без кліку. Кнопка
лишається на місці як і раніше (ручний фолбек, і водночас сам сентинел). Захист від повторного
спрацювання на той самий `href`, поки він у зоні видимості, — інакше нескінченний потік однакових
навігацій під час однієї прокрутки. Стосується всіх 9 сторінок, що використовують `LoadMore`
(гравці, матчі, турніри, новини й адмінські списки) — без жодної правки в самих сторінках.

Живою перевіркою (`npm run dev`, реальна БД) підтверджено на `/players`: прокрутка кнопки в зону
видимості (без кліку) сама підвантажує наступну сторінку (`?show=20` → `?show=40`). 7 нових
тестів у `tests/components/load-more.test.tsx` (фейковий `IntersectionObserver` + мок
`next/navigation`).

## 2026-08-11 — Команди та зустрічі (тайми) для змішаних турнірів

Новий опціональний флоу для MIXED-турнірів: адмін може розбити учасників на команди по 2-4
гравці; дві команди грають "зустріч" (тай, за зразком Кубка Девіса) з кількох окремих поєдинків
(рабберів — звичайних одиночних/парних матчів), і команда з більшістю виграних рабберів виграє
зустріч. Повністю адитивно — див. `docs/TOURNAMENT_TEAMS.md` для дизайну й переліку файлів, що
лишились без жодної зміни (`matchFormSchema`/`createMatchAction`/`MatchDialog`,
`tournament-standings.ts`, `standings-sort.ts`, `rating/*`).

Нова Prisma-міграція (`20260811134059_add_tournament_teams_and_ties`) — три нові таблиці
(`TournamentTeam`, `TournamentTeamMember`, `TournamentTie`) + один nullable `Match.tieId`, без
жодної деструктивної зміни наявних таблиць. Новий адмінський таб "Команди" (лише для MIXED-
турнірів), нова секція "Командний залік"/зустрічі на публічній сторінці турніру (рендериться,
лише коли є хоч одна зустріч). 61 новий тест (`tests/lib/validation/rubber.test.ts`,
`tests/lib/actions/teams.test.ts`, `tests/lib/actions/ties.test.ts`,
`tests/lib/tournament-ties.test.ts`, плюс тести нових компонентів) — повний набір: 1037/1037,
включно з усіма наявними тестами, що пройшли без жодної правки.

## 2026-08-11 — Коло "VS" на картці матчу тепер саме є тенісним м'ячем

На прохання користувача два окремі 🎾 над і під колом "VS" (`match-card-image.tsx`) прибрано —
натомість саме коло навколо "VS" тепер і є м'ячем: емодзі 🎾 у великому розмірі як фон, напис
"VS" білим поверх нього. Два уточнення від користувача після першого варіанту: (1) м'яч мав
`alignSelf: "stretch"`-поведінку за замовчуванням (фіксована `height` скасовує `stretch`, тож він
прилипав до верху ряду) — виправлено `alignSelf: "center"`, тепер він на рівні гравців, а не
зверху картки; (2) природний яскравий жовто-зелений колір емодзі затемнено напівпрозорим
`rgba(6,36,20,0.45)` колом поверх нього (Satori не вміє перефарбовувати саме emoji-гліф, тож це
обхідний шлях, задокументований коментарем у коді).

## 2026-08-11 — Тести на пропущені раніше місця нового функціоналу

Перевірка: чи всі нові фічі (стрілка тренду рангу, картки-шер) вже мають тести. Знайдено дві
прогалини:
- `tournamentShareCaption` (`src/components/tournament-standings.tsx`, тепер експортована) —
  текст Web Share для підсумків турніру — не мав жодного тесту. Додано тести на порядок топ-3,
  ігнорування рядків без місця, і тест-рендер, що показує кнопку "Поділитися" лише коли турнір
  `complete` і відомі `tournamentId`+`tournamentName` одночасно.
- `getSinglesRatingsTrend`/`getDoublesRatingsTrend`/`getSinglesSetClubTrend`/
  `getDoublesSetClubTrend` (`src/lib/rating/ratings-data.ts`) — самі складові
  (`excludeLatestTournament`, `buildRankDeltaMap`, рушії рейтингу) вже були перевірені окремо, але
  саме "склеювання" — жодного тесту. Додано тести на пропуск дебютанта останнього турніру (немає
  запису "до"), і найважливіше — тест на нюанс із доккоменту, який досі ніхто не перевіряв:
  виключається найновіший турнір **у межах запитаного сезону**, а не найновіший за весь час клубу.

`tests/lib/rating/ratings-data.test.ts` (+4 тести), `tests/components/tournament-standings.test.tsx`
(+4 тести). Повний набір: 976/976.

## 2026-08-11 — Тайбрейк у тексті "Поділитися результатом"

Користувач: якщо в сеті був тайбрейк, його рахунок теж має потрапляти в текст повідомлення, не
лише на саму картку. `matchShareCaption` (`src/components/match-summary.tsx`, тепер
експортована для прямого юніт-тестування) додає рахунок брейка в дужках одразу після сету —
`7:6(7:5)` (спочатку очки переможця, як і решта підпису), той самий принцип, що вже показує сама
картка (обидва боки бачать свій рахунок брейка, не лише той, хто програв). Чотири нові тести в
`tests/components/match-summary.test.tsx`, включно з точним відтворенням реального матчу з
тайбрейком з бойової бази.

## 2026-08-11 — Тенісні м'ячики замість ліній-розділювачів на картці матчу

На прохання користувача дві тонкі лінії навколо кружечка "VS" (`match-card-image.tsx`) замінено
на 🎾. Перша спроба (кожен м'яч у власному `flex: 1` контейнері з `alignItems`/`justifyContent`,
щоб зайняти рівно половину висоти по обидва боки від "VS") зламала весь блок — кружечок "VS" і
другий м'яч зникли, перший опинився далеко за межами картки. Причина не досліджувалась глибше
(ще одна Satori-специфічна пастка з flex-розрахунком висоти при вкладеному emoji), просто
уникнено: емодзі тепер прямі діти колонки (без обгортки), сама колонка без `flex`, центрування —
через `justifyContent: "center"` на колонці, що й так розтягнута на всю висоту рядка (`alignItems:
"stretch"` за замовчуванням від батьківського ряду) — той самий фінальний результат, без
вкладеного flex. Перевірено вживу.

## 2026-08-11 — Фікс: повідомлення "Поділитися" в месенджері показувало заголовок діалогу, а не результат

Користувач помітив: коли ділишся карткою через Web Share API (мобільний "Поділитися…"), у
WhatsApp/Telegram як текст повідомлення підставлявся заголовок діалогу ("Поділитися результатом
матчу") замість реального результату. Причина: `ShareResultButton` передавав `title` напряму в
`navigator.share({files, title})`, а більшість Android-застосунків при шерингу файлу підставляють
повідомлення з поля `text`, не `title`. Додано окремий обов'язковий пропс `shareText` —
`title` лишається лише заголовком діалогу, `text` іде в сам `navigator.share()`.
`matchShareCaption` (`src/components/match-summary.tsx`) будує "Хіщенко Олександр переміг
Іоганов Денис 7:5 (20.06.2026)" (нова гендерна `wonVerb` в `src/lib/player-display.ts`, за тим
самим принципом, що вже `retiredLabel`); `tournamentShareCaption`
(`src/components/tournament-standings.tsx`, тепер приймає `tournamentName`) будує "1. X, 2. Y,
3. Z — Підсумки турніру «назва»"; сезонна кнопка (`src/app/rating/page.tsx`) — просто "{рік} рік
у SET.club". Перевірено вживу через Playwright з підміненим `navigator.share` — обидва реальні
виклики (матч і турнір) підтверджені: `title` не передається взагалі, `text` — природне речення.

## 2026-08-11 — Реальні Google-аватарки на картках "Поділитися результатом" замість ініціалів

Користувач запитав, чи можна показувати справжнє фото гравця (з Google-акаунту) замість кружечка
з першою літерою. `matchWithDetailsInclude` (`src/lib/queries/matches.ts`) тепер вибирає
`player.user.image` — той самий Google avatar URL, що вже показує `<AvatarImage>` на `/rating`
і сторінці гравця, просто раніше не потрапляв у вибірку матчу. `MatchShareSide.players:
{name, image}[]` (перейменовано з `names: string[]`), `InitialAvatar`
(`src/lib/share/card-chrome.tsx`) рендерить `<img>`, коли фото є, інакше лишається попередній
fallback-кружечок з ініціалом — гравці без прив'язаного Google-акаунту не ламаються. Колір
переможця (золото) — тепер обідок навколо фото замість заливки кружечка. Перевірено вживу на
реальному матчі з одним гравцем із фото й іншим без — обидва варіанти в одній картці.

## 2026-08-11 — Фікс: бренд-марка на картках-шер показувала вигадану "S." замість справжнього логотипу

Замінено на той самий двострічковий wordmark "SET." / "club", що вже на фавіконі й у навбарі
(`src/components/logo.tsx`, `src/lib/brand-icon.tsx`) — попередня версія мала власну, ніде більше
не використану абревіатуру.

## 2026-08-11 — Візуальний редизайн карток "Поділитися результатом"

Користувач: перша версія карток (флет діагональний градієнт, дві тонкі білі лінії) виглядала
застаріло. Спільна для всіх трьох карток частина (фон/шапка/підвал/аватарки) винесена в новий
`src/lib/share/card-chrome.tsx` — фон тепер темна база з двома м'якими radial-gradient
"світляками" замість флет градієнта; шапка отримала градієнтну бренд-марку й контекстну
"пігулку"; матчева картка — кругові аватарки-ініціали (золото для переможця, приглушено для
того, хто програв), значно більший і контрастніший рахунок сетів, сторони "дивляться" одна на
одну через центральний розділювач із міткою "VS"; подіум турніру — справжні "сходинки" різної
висоти; річна картка — статистика як великі числа-акценти. `match-card-image.tsx`,
`tournament-card-image.tsx`, `season-card-image.tsx` переписані на нову спільну "хрому".
Живий перегляд (curl+PNG) для одиночного матчу з тайбрейком, парного матчу, матчу зі "Знявся з
матчу", подіуму турніру й річної картки — усе відрендерилось коректно з першої спроби. Детальніше
— `docs/SHARE_CARDS.md`.

## 2026-08-11 — Прибрано дублювання плей-офф матчів у публічній картці турніру

На `/tournaments/[id]` матчі плей-офф (Фінал, За 3 місце тощо) показувались двічі: один раз у
власній секції «Плей-офф» (`TournamentPlayoffs`, згруповано по раунду), і ще раз у плоскому
списку «N матчів» нижче — той самий `matches` масив рендерився без фільтра. Ця публічна сторінка
не має жодного інтерфейсу для редагування матчів (на відміну від адмінського таба «Матчі», який
залишили без змін — це справжня поверхня керування), тож дублювання було суто візуальним шумом.
Плоский список тепер фільтрує `!isPlayoffRound(match.round)` — рахує й показує лише матчі
групового етапу; лічильник (17 → 15) теж змінився відповідно.

## 2026-08-11 — Фікс: матчі плей-офф зайво потрапляли в таблицю ДО плей-офф

Для турніру без «За групами»/сіяності (одна умовна група на весь турнір) таблиця учасників,
що показується НАД «Підсумковою таблицею», рахувала кожному гравцеві його матчі плей-офф
(Фінал, За 3 місце тощо) в загальну статистику Матчів/Перемог/Очок разом із груповим етапом —
тому вона майже дублювала «Підсумкову таблицю» замість того, щоб показувати чистий результат
групового етапу. `getTournamentStandingsRows` (`src/lib/tournament-standings.ts`) тепер рахує
цю таблицю через `buildScopedSinglesRows` (той самий підхід, що вже в «За групами»/«За
сіяністю»/додаткових групах) — плейофф-раунди виключаються (`isPlayoffRound`). «Підсумкова
таблиця» (`placedTable`) і далі рахує все разом, як і раніше — це навмисно (підсумковий
результат турніру), змінилась лише таблиця, що передує плей-офф. Побічно прибрано мертвий
`h2h`, який рахувався в `getIndividualRows`, але після цієї зміни ніде вже не читався.

## 2026-08-11 — Фікс: гравці без плей-офф лишались без місця в «Підсумковій таблиці»

У турнірі з однією групою (без «За групами»/додаткових груп), де плей-офф зіграли лише перші
N учасників, ті, хто не потрапив у плей-офф, показувались із «—» замість реального місця —
навіть коли вони вже зіграли повний коловий турнір одне проти одного в груповому етапі і їхній
взаємний результат насправді відомий (приклад: турнір `20.06.2026`, Андрєєв Олександр і
Теребіж Віктор мали «—» замість 5 і 6 місця). `buildGeneralPlacedTable`
(`src/lib/tournament-standings.ts`) вже мала подібний фолбек лише для іменованої «Додаткової
групи» (`createTournamentGroupAction`) — тепер той самий принцип застосовується до БУДЬ-ЯКИХ
гравців, що лишились без місця: якщо їхній власний коловий турнір одне проти одного (без
плей-офф-раундів) уже завершений (`isRoundRobinComplete`), вони отримують наступний блок місць
за своєю таблицею. Безпечно для випадку з докстрінгу («не вигадувати місце для тих, хто ніколи
не грав одне проти одного») — якщо частина «залишку» насправді з різних груп і не грала між
собою, `isRoundRobinComplete` поверне `false`, і вони так і лишаться з «—», як і раніше.
Одинокий гравець без пари для порівняння (рівно 1 залишок) — те саме відоме обмеження, що вже
було для одноосібної «Додаткової групи». Два нові тести в
`tests/lib/tournament-standings.test.ts`: залишок отримує місця, коли вже грав між собою;
залишок лишається без місця, коли ні.

## 2026-08-11 — Фільтр за форматом на сторінці «Турніри»

На `/tournaments` додано сегментований фільтр «Усі / Одиночні / Парні» (`PillFilterGroup`, той
самий компонент, що вже на `/rating`) — раніше картки одиночних і парних турнірів завжди йшли
впереміш в одній стрічці. `getTournamentsPage` (`src/lib/queries/tournaments.ts`) отримала
четвертий необов'язковий параметр `format: TournamentFormat`, що додає `where.format` до
запиту. Турніри формату `MIXED` (рідкісний, не має власної вкладки й на `/rating`) лишаються
видимі лише під «Усі», без окремого пункту. Сторінку `/admin/tournaments` не чіпали — запит на
фільтр стосувався саме публічної сторінки «Турніри».

## 2026-08-10 — Фікс: стрілка зміни рангу зʼїжджала під номер місця

На `/rating` кружечок з номером місця мав клас `flex` — блоковий flex-контейнер, що займає
всю ширину комірки, тож стрілка `▲/▼` (сусідній inline-елемент) переносилась на новий рядок
під номером, а не лишалась поруч. Виправлено обгортанням кружечка й стрілки у спільний
`<span className="flex items-center">` в обох таблицях `src/app/rating/page.tsx` (SET.club і
Glicko-2/OpenSkill). Перевірено живим скріншотом — стрілка тепер одразу праворуч від номера
(`3 ▲1`).

## 2026-08-10 — Три статистичні фічі: стрілка зміни рангу, найкращий партнер, річний підсумок

Продовження обговорення "чого бракує застосунку" зі статистичного боку — з короткого списку ідей
користувач обрав три: стрілку зміни місця в рейтингу, найкращого партнера в парних матчах на
профілі гравця, і річний підсумок сезону як картку-шер (розширення фічі "Поділитися результатом"
вище). Уточнено заздалегідь: стрілка порівнює з **минулим турніром** (не фіксованим періодом),
річний підсумок — **лише картка**, без нової сторінки.

**Стрілка зміни рангу** (`src/lib/rating/rank-trend.ts`) — увесь рейтинговий рушій і без того
рахує все "з нуля" від сирих матчів при кожному запиті, тож "минуле" місце — це та сама таблиця,
порахована без матчів останнього турніру (`excludeLatestTournament`). Чотири нові функції в
`src/lib/rating/ratings-data.ts` (`get{Singles,Doubles}RatingsTrend`,
`get{Singles,Doubles}SetClubTrend`), новий `RankTrendArrow` (`▲N`/`▼N`), показується на `/rating`
(обидві таблиці) і на профілі гравця (`RatingCard`). Дебютант (немає в минулій таблиці) — без
стрілки, не "0".

**Найкращий партнер** (`src/lib/best-partner.ts`, `findBestPartner`) — чиста пост-обробка вже
завантажених на профілі гравця матчів (`getPlayerMatches`, жодного нового запиту): партнер із
найкращим win% у парних матчах (тай-брейк — більше спільних матчів), той самий walkover-виняток,
що й `summarizePlayerStats`. Картка на `/players/[id]`, лише коли є парна історія.

**Річна картка** (`src/lib/share/season-card-data.ts`/`season-card-image.tsx`,
`GET /api/share/season/[year]`) — пряме розширення інфраструктури "Поділитися результатом":
кількість матчів/турнірів за рік (два нові прямі `prisma.count`-запити,
`getSeasonMatchCount`/`getSeasonTournamentCount`) + топ-1 гравець сезону в кожному форматі (вже
наявні `get{Singles,Doubles}SetClubPoints(year)`). Кнопка (уже наявний, дженерик
`ShareResultButton`, без змін у ньому) — на `/rating`, з'являється лише коли обрано конкретний
календарний рік. Повний дизайн — [docs/SHARE_CARDS.md](SHARE_CARDS.md), [docs/RATING.md](RATING.md).

Живий перегляд (Playwright + `curl`, тимчасові скрипти видалено після перевірки): стрілки на
`/rating` коректні в обох моделях (SET.club і Glicko-2/OpenSkill), картка партнера на реальному
профілі показує вірний W-L, кнопка річного підсумку з'являється лише для конкретного року й
відкриває робочу картку 1200×630 (кирилиця, коректні числа).

`tsc --noEmit`, `npx vitest run` (955 тестів, +16 нових), `npm run lint`, `npm run build` — усі
чисті.

## 2026-08-10 — «Поділитися результатом» — картки для соцмереж (матч і подіум турніру)

Глибоке дослідження застосунку (окрема сесія) виявило кілька продуктових прогалин поза межами
попередніх code review/UX-аудитів. З короткого списку ідей користувач обрав одну — можливість
поділитися результатом матчу чи фінальними місцями турніру як готовою картинкою для соцмереж —
і після перегляду HTML-мокапу (Artifact) попросив реалізувати. Повний дизайн-документ —
[docs/SHARE_CARDS.md](SHARE_CARDS.md).

Нова кнопка «Поділитися» на завершеному матчі (`MatchSummary`) і на вирішеній «Підсумковій
таблиці» турніру (`TournamentStandingsSection`) відкриває діалог з прев'ю картки 1200×630,
кнопками «Завантажити» (завжди) і «Поділитися…» (Web Share API, лише мобільні браузери з
підтримкою file-sharing). Зображення генерується на сервері через `ImageResponse` з `next/og`
(`GET /api/share/match/[id]`, `.../tournament/[id]`) — той самий підхід, що вже застосований до
`src/app/icon.tsx`/`apple-icon.tsx`/`manifest-icons`, вперше для реальних даних, а не статичного
логотипу.

Кожна картка розділена на чисту функцію даних (`src/lib/share/match-card-data.ts`,
`.../tournament-card-data.ts` — тестовані) і окрему JSX-функцію для `ImageResponse`
(`match-card-image.tsx`, `tournament-card-image.tsx` — не тестуються, той самий стандарт, що вже
мовчки застосований до `brandIconElement`). Подіум турніру показує W-L рекорд, не бали SET.club
(`PlacedStandingsRow.points` — це турнірні "Очки" з `computeMatchPoints`, зовсім інша система
підрахунку, ніж клубний рейтинг — на мокапі це було помилково підписано "балів SET.club").

**Знайдено й виправлено живим переглядом (не ловиться tsc/тестами):** `inset: 0` не працює в
Satori (двигуні `ImageResponse`) — фоновий градієнт і весь білий текст рендерились невидимими,
доки не замінили на явні `top/left/right/bottom: 0`. Кирилиця відрендерилась коректно одразу
після фіксу — перевірено на реальних іменах з бойової БД через прямий `curl` + перегляд PNG.

`normalizeRoundLabel`/`MATCH_TYPE_LABEL` винесено з `match-summary.tsx` у новий
`src/lib/match-display.ts` (business logic, не UI) — потрібні і компоненту, і новій
`match-card-data.ts`, а `lib/` не повинен імпортувати з `components/`.

`tsc --noEmit`, `npx vitest run` (939 тестів, +15 нових: `buildMatchShareData`/
`buildTournamentShareData` — успішні/null-кейси, `ShareResultButton` — прев'ю/завантаження/Web
Share/скасування), `npm run lint`, `npm run build` — усі чисті. Живий перегляд: Playwright проти
реального dev-сервера (тимчасовий скрипт, видалений після перевірки) підтвердив реальне
завантаження зображення (`naturalWidth === 1200`) і реальну подію завантаження файлу з очікуваним
іменем.

## 2026-08-10 — Одиночний Set Club тепер рахується за формулою, як і парний

Одиночний Set Club (`/rating?format=singles&model=setclub`) рахував бали за фіксованою таблицею
(10/8/6/5/4/3/2, потім плоско 1) + окремий бонус за розмір поля (+1/+2). Користувач надав таблицю
балів для 5-12 учасників — вона виявилась рівно тією ж формулою, що вже рахує парний Set Club:
`2 × (N − k + 1)` балів за місце k із N зареєстрованих учасників, без потреби в окремому бонусі
(масштабується сама на будь-яку кількість гравців).

- `src/lib/rating/placement.ts` — `placePoints(place, total)` (спільна формула, була дубльована
  лише в `setclub.ts`) і `fillRemainingPlacements` (виділений "добрати решту місць"-хвіст
  `resolvePlacements`, щоб `setclub-singles.ts` міг вставити свій крок між декісив-плейоффом і
  фолбеком — див. нижче).
- `src/lib/rating/setclub-singles.ts` — прибрано `PLACE_POINTS`/`basePlacePoints`/
  `fieldSizeBonus`, тепер `placePoints(place, participantCount)`.
- **Побічний фікс, знайдений під час перевірки формули**: для формату "4 групи по 3 + плейофф"
  (GROUPS_12_PLAYOFF) місця 9-12 раніше падали в загальний (не scoped) fallback, що ранжував
  четвірку 3-х-місних фіналістів груп за статистикою **всього турніру** — нешкідливо зі старою
  плоскою таблицею (усі однаково 1 бал), але не з новою формулою (різниця 9-е vs 12-е місце при
  N=12 — 6 балів). `resolveSinglesPlacements`/`buildScopedStandings` тепер ранжують цю четвірку
  лише за 6 матчами самої міні-групи (`MINI_GROUP_ROUND`) — той самий скоуп, що вже є для
  відображення в `tournament-standings.ts`. Новий тест явно ловить регресію (звичайного
  "групового шуму" достатньо, щоб змінити порядок 9-10 місця, якщо скоуп прибрати).
- `src/app/rating/page.tsx` — текст пояснення "Як нараховуються бали за місце" уніфіковано з
  парним варіантом; окрема секція "Чому враховується кількість учасників" (стара, про бонус)
  прибрана для одиночних.
- `docs/RATING.md`/`docs/GROUPS12_PLAYOFF.md` — секція одиночного Set Club переписана, стара
  версія лишена згорнутою для історії.
- Перевірено вживу: `npm run dev` + Playwright-скрипт проти реальної БД — обидва акордеони на
  `/rating` показують новий текст, живі числа в таблиці (42/30/16/14... для одиночних) вже
  парні, як і очікує нова формула.

## 2026-08-10 — Перевірка покриття тестами: 6 реальних прогалин закрито + знайдено й виправлено баг

Проаналізував точні непокриті рядки (не просто відсоток) по всій кодовій базі, відсіявши
"прийнятні" прогалини (захисні `throw error` fallback-и) від реальних незвіреної логіки:

- **`tournament-gallery.tsx` — тесту не було взагалі** (0% покриття). Новий
  `tests/components/tournament-gallery.test.tsx`.
- **`csv-response.ts` — тесту не було, і тест одразу знайшов реальний баг**: `Content-Disposition`
  з кириличним іменем файлу (напр. ім'я турніру/гравця) кидає `TypeError` при побудові
  `NextResponse` (заголовки — лише Latin-1 ByteString), тобто впав би у 500 весь export-роут.
  Наразі всі викликачі передають лише ASCII-імена (`matchi-...`, `uchasnyky-...`), тож це
  латентна, а не діюча проблема — але `csvResponse` спільна утиліта, тож не варто покладатись на
  це назавжди. Виправлено: `Content-Disposition` тепер несе і ASCII-fallback (`filename=`), і
  RFC 5987 percent-encoded UTF-8 (`filename*=`) — стандартний, підтримуваний усіма браузерами
  спосіб.
- **`createTournamentGroupAction`'s розрізнення двох P2002** (гонка номера групи vs дубль
  гравця) — нові тести в `tests/lib/actions/tournaments.test.ts`.
- **`deleteMatchAction`'s каскадний ресет** (fills/resets нижче по сітці після підтвердження) —
  новий тест у `tests/lib/actions/matches.test.ts`.
- **`saveScoreAction`'s гонка "матч видалили, поки форма рахунку відкрита"** — новий тест.
- **`linkPlayerAction`'s весь error-шлях** (юзера видалили, FK-гонка) + **`updatePlayerAction`'s
  валідація** — нові тести в `tests/lib/actions/players.test.ts`.

Покриття: 93.14→93.63% stmts, 85.34→85.86% branches, 907→920 тестів. Повний набір + `npm run
build` + `npx tsc --noEmit` — усе зелене.

## 2026-08-10 — Повне рев'ю застосунку: усунення знахідок

Повторне глибоке рев'ю (п'ять паралельних агентів, кожен свій шар) — див.
[docs/CODE_REVIEW_2026-08-10.md](CODE_REVIEW_2026-08-10.md) за повним звітом. Усі 08-08-фікси
підтверджено коректними; нижче — усунення нових знахідок того ж дня.

- **Ключ обкладинки новини тепер прив'язаний до посту** — `NewsPost.photoKey` отримав
  `@unique` (`prisma/schema.prisma`, нова міграція
  `20260810120000_news_post_photo_key_unique`). Раніше адмін міг підставити чужий (публічно
  видимий) ключ фото іншого посту — обидва пости вказували б на один R2-об'єкт, і видалення/
  заміна одного назавжди ламало обкладинку іншого. Тепер повторне використання ключа падає
  дружньою помилкою "Це фото вже використовується в іншій новині" (`src/lib/actions/news.ts`,
  нові гілки `isUniqueConstraintError` у create/update).
- **`confirmPhotoUploadAction` тепер обробляє повторний confirm того самого ключа** —
  бракувала гілка `isUniqueConstraintError` поряд з уже наявною `isForeignKeyError`
  (`src/lib/actions/photos.ts`) — раніше retried-запит на вже підтверджений ключ падав
  непійманим P2002 замість "Це фото вже завантажено".
- **"Додаткова група" більше не блокує місце всім, якщо знятий учасник не встиг зіграти всі
  свої матчі до withdrawal** — `buildGeneralPlacedTable` (`src/lib/tournament-standings.ts`)
  тепер прибирає знятого з `members` перед перевіркою `isRoundRobinComplete`, а не вимагає
  його власної повної історії проти всіх. Раніше активні, повністю відіграні учасники такої
  групи назавжди лишались без `place`, якщо адмін не встиг створити матч між знятим і кимось
  із групи.
- **Витік `ObjectURL` у полі обкладинки новини** (`src/components/admin/news-photo-field.tsx`)
  — кожен вибір файлу створював новий blob-прев'ю без `revokeObjectURL` на попередній. Тепер
  ref-трекінг власного blob URL з `revokeObjectURL` при заміні, прибиранні фото й на unmount.
- **«Читати повністю» в картці новини тепер перерахунковується при resize**
  (`src/components/news-card.tsx`) — раніше `overflowing` рахувався лише раз при монтуванні;
  текст, що не переповнював клемп на широкому екрані, лишався без кнопки після звуження вікна.
- **Флейкі-тест `tournament-roster.test.tsx` — фікс кореневої причини, не ще один таймаут** —
  об'єднано окремий `waitFor`+`findByText` (два послідовні таймаут-вікна) в один `waitFor`,
  що перевіряє виклик моку й фінальний DOM-стан разом; прибрано `retry: 2`, який ховав би нову
  флейкі-регресію від швидкого погляду на CI. Додатково: `vitest.config.ts` тепер обмежує
  `maxWorkers: 2` — корінь проблеми був у перевантаженні CPU паралельними воркерами, що
  зсувало React-scheduler timing, а не в самій асерції.
- **GROUPS12: задокументовано (не виправлено логікою)** екстремальний варіант withdrawal-кейсу
  — зняття 3-х з 4-х рангів-3 залишає в міні-групі "9-12" лише 1 реального кандидата, який
  ніколи не отримає `place` (`isRoundRobinComplete` вимагає 2+ рядки). Свідомо не фіксовано:
  коректний фікс вимагає тягнути статус завершеності всіх 4 груп у
  `buildGroups12PlayoffTable`, ризик хибно зафіксувати `place` заранo для настільки рідкісного
  сценарію не виправдав зусиль — див. `docs/GROUPS12_PLAYOFF.md`'s "Відомі обмеження".
- Нові тести: `tests/lib/actions/news.test.ts`, `tests/lib/actions/photos.test.ts` (unique-
  конфлікти), `tests/lib/tournament-standings.test.ts` (withdrawal-до-всіх-матчів у кастомній
  групі). Повний набір (907 тестів) + `npm run build` + `npx prisma validate` — усе зелене.

## 2026-08-10 — Клік на «Перемоги»/«Поразки» у профілі гравця фільтрує історію матчів

Плитки «Перемоги»/«Поразки» на профілі гравця були суто інформаційними — жодного способу
подивитись саме ці матчі, не гортаючи всю історію вручну.

`StatCard` (`src/components/stat-card.tsx`) отримав опційні `href`/`active` — з `href` уся
плитка стає посиланням (`Link` навколо `CardContent`), з `active` підсвічується рамкою. Плитка
«Матчі» скидає всі фільтри, «Перемоги»/«Поразки» — перемикають `?result=win|loss` (клік по вже
активній знову — знімає фільтр). Новий `matchResultForPlayer` на сторінці профілю рахує
перемогу/поразку для гравця точно так само, як `summarizePlayerStats` (виключає walkover-сторону
того, хто знявся), щоб лічильники на плитках і список під ними завжди узгоджувались.

Заразом, за проханням користувача: коли активний фільтр перемог/поразок, з'являються ще два
ряди `PillFilterGroup` (той самий компонент, що на `/leaderboard`) — «Формат» (Усі/Одиночні/
Парні) і «Рік» (роки рахуються лише з "вирішених" матчів цього гравця, не club-wide
`getResultYears`). Усі фільтри (суперник/результат/формат/рік) комбінуються через AND і
зберігаються один при одному при перемиканні — `OpponentFilter` тепер приймає `type`/`year` й
не губить їх, коли міняється суперник.

Перевірено live (Playwright, реальний гравець із 29 матчами): клік «Перемоги» — 29→11 карток
(рівно `stats.wins`), + «Одиночні» — 11→7, + рік 2026 — 7→4, кожен крок коректно накопичує
query-параметри.

`tsc --noEmit`, `npx vitest run` (903 тести, +6 нових: `StatCard` як лінк/актив, `OpponentFilter`
зберігає result/type/year), `npm run lint`, `npm run build` — усі чисті.
...
`overflow-x-hidden` поруч явно — і скролбар усе одно лишився, зі скріншотом користувача, що
показав його точно поруч із двома іконками завантаження. Живо перевірив
`getComputedStyle(ul).overflowY` — і він доводив `"auto"`, НЕ `"visible"`, попри явний
`overflow-y-visible` у className.

Причина — специфікація CSS Overflow: якщо одна вісь (`overflow-x`) НЕ `visible`, а інша
(`overflow-y`) — `visible`, браузер примусово обчислює `visible` як `auto` для сумісності —
і це відбувається незалежно від того, чи `visible` вказано явно, чи лишено за замовчуванням.
Поєднати `hidden` по одній осі з `visible` по іншій на тому самому елементі просто неможливо —
моє попереднє припущення, що явне вказання обох осей уникає цього правила, було хибним.

Фікс: прибрано `overflow-x`/`overflow-y` з `<ul>` повністю, з обох осей. Жодна вісь тут
насправді й не потрібна: по горизонталі текст файлу вже сам себе обрізає через `truncate`
(Tailwind-клас, що вже включає власний `overflow: hidden` на `<span>`) — списку нема чого
обрізати самому. По вертикалі, як і раніше, приймається природний ріст, а довгий пакет ловить
скрол `DialogContent`.

Перевірено live саме зі сценарієм користувача (2 файли одночасно, семпли кожні 100мс) —
`getComputedStyle` тепер справді показує `overflowY: "visible"` на `<ul>`, і жодного скролбару
на скріншоті. Повторно перевірено на 15 файлах — жодної регресії, довгий пакет так само не
потребує скролу в межах 85vh діалогу.

`tsc --noEmit`, `npx vitest run` (897 тестів), `npm run lint`, `npm run build` — усі чисті.

## 2026-08-10 — Справжня причина вертикального скролу в списку файлів (не пов'язана з горизонтальним)

Користувач повідомив, що після фіксу горизонтального скролу лишились вертикальні стрілки
вгору/вниз біля іконки завантаження. Живо (Playwright, реальний браузер, реальна dev-база)
семплував `<ul>` списку файлів кожні 120мс під час завантаження ОДНОГО файлу — і виявив, що
`scrollHeight` нестабільно чергується між 20 і 21px, поки рядок (`getBoundingClientRect().height`)
незмінно лишається 20px, у тому числі вже ПІСЛЯ завершення (стан "settled"). Це не пов'язано з
кількістю файлів, зумом чи анімацією спіна (усе перевірено й відкинуто живо) — класична
розбіжність округлення браузера між `scrollHeight` (за вмістом) і `clientHeight` (за
розкладкою) для `line-height`, що не потрапляє точно на піксель. Спроба дати "запас" через
`max-h-72` замість `max-h-60` не спрацювала (перевірено live) — `clientHeight` визначається
вмістом (20px), а не кепом, тож запас над кепом нічого не міняє.

Фікс: прибрано внутрішній скрол-контейнер списку повністю (`overflow-y-visible` + без
`max-h-*`) — список тепер природно росте, а по-справжньому довгий пакет файлів ловить власний
`overflow-y-auto`/`max-h-[85vh]` `DialogContent` (там 1px шум ніколи не має значення — сумарний
контент набагато більший). `overflow-y-visible` — CSS-гарантія: на відміну від `auto`, ніколи
не показує скролбар незалежно від будь-якого piksel-шуму. `overflow-x-hidden` лишив явним
поруч (а не залишив неявним) — інакше специфікація CSS знову змусила б протилежну вісь стати
`auto`. Перевірено live і з 1, і з 15 файлами одночасно — жодного скролбару в жодному випадку.

`tsc --noEmit`, `npx vitest run` (897 тестів), `npm run lint`, `npm run build` — усі чисті.

## 2026-08-10 — Живе розслідування двох повторних скарг на скрол (браузер, не лише код)

Користувач повідомив, що обидва попередні фікси скролу не спрацювали. Замість продовжувати
гадати з коду, підняв `npm run dev`, тимчасово додав `E2E_TEST_LOGIN_SECRET` у `.env.local`
(локально, у `.gitignore`, вимкнено на продакшені незалежно від цього) і провів живе
розслідування через Playwright (`playwright-core`, напряму, без `chromium-cli` — недоступний
у цьому середовищі) проти справжньої dev-бази — з перехопленням мережевих запитів до
`*.r2.dev`, щоб підставити власне контрольоване дуже високе (900×3600, портрет 1:4) синтетичне
зображення замість повільного реального оригіналу.

**Горизонтальний скрол у `PhotoUploadDialog` (завантаження файлу):** живо підтвердив реальне,
хоч і крихітне (~3px, нестабільне між замірами, зникає після статусу "Готово"/помилка)
переповнення рядка САМЕ під час стану "uploading" (поки крутиться `Loader2Icon`) — щось у
розкладці під час анімації спіна не збігається день-у-день з попереднім фіксом (`min-w-0`).
Причина лишається до кінця не з'ясованою (можливо, часовий/анімаційний шум вимірювання,
не стабільний баг розкладки), але фікс тепер гарантований незалежно від причини: додано
`overflow-x-hidden` до самого `<ul>` (раніше лишався неявним `auto`, бо `overflow-y-auto` за
специфікацією CSS автоматично вмикає це і для X) — горизонтальний скрол тепер неможливий
структурно, а не просто малоймовірний.

**«Скрол при натисканні на хрестик» у лайтбоксі:** живо виміряв — це НЕ баг. Сторінка турніру
справді має висоту ~6490px проти viewport ~900px (реальний, легітимний скрол сторінки). Поки
лайтбокс відкритий, `Dialog` коректно блокує `body { overflow: hidden }`; клік на "Закрити"
знімає блокування, і власний скролбар СТОРІНКИ (не лайтбоксу) одразу повертається — саме тому
це виглядає, ніби "хрестик викликає скрол". Це стандартна, очікувана поведінка будь-якого
модального вікна — прибрати її означало б назавжди заблокувати скрол сторінки після першого ж
відкриття лайтбоксу, що є явно гіршим багом. Нічого не змінено тут навмисно.

Синтетичні портретні зображення (до співвідношення 1:4, вищі за будь-яке реальне фото) на
висотах viewport 700-900px і масштабуванні екрана 150% не показали жодного переповнення
`DialogContent` — попередній фікс (`max-h-[70vh]`) підтверджено живо працює.

`tsc --noEmit`, `npx vitest run` (897 тестів), `npm run lint`, `npm run build` — усі чисті.

## 2026-08-10 — Фікс: спурний вертикальний скрол у лайтбоксі фото для портретних фото

Користувач повідомив про скролбар, що з'являвся біля кнопки закриття (✕) лайтбоксу. Причина та
сама родина бага, що й у `PhotoUploadDialog` раніше (записи вище): базовий `DialogContent` має
`max-h-[85vh]` з `overflow-y-auto`, а фото в лайтбоксі — `max-h-[80vh]`. Для портретного фото
на висоті 80vh лишалось лише ~5vh запасу для рядка кнопок (нав./видалити/закрити) під ним — не
завжди достатньо на кожній висоті вікна, тож увесь діалог (фото + кнопки разом) ставав
прокручуваним, показуючи нестильований нативний скролбар і зсуваючи кнопку закриття до/за межу
видимої області.

Фікс: зменшено `max-h-[80vh]` фото до `max-h-[70vh]` — лишає стабільний запас (~15vh) для рядка
кнопок, тож увесь вміст завжди вміщується під кепом `DialogContent`, скролбар більше не потрібен.
Кеп і `overflow-y-auto` на самому `DialogContent` свідомо не чіпав — лишається захистом на
випадок справді крихітних вікон, а не прибирається повністю (без нього кнопка закриття могла б
стати недосяжною на екстремально низьких вікнах, без жодного способу до неї доскролити).

`tsc --noEmit`, `npx vitest run` (897 тестів — той самий набір, лайтбокс-тести й далі зелені),
`npm run lint`, `npm run build` — усі чисті.

## 2026-08-10 — Підтвердження перед видаленням фото турніру + більші кнопки в лайтбоксі

Кнопка «Видалити фото» в `PhotoLightbox` видаляла одразу по кліку, без жодного підтвердження —
на відміну від решти видалень у застосунку (новини, учасники турніру тощо), які завжди питають.
Заразом користувач повідомив, що кнопки навігації/видалення/закриття в лайтбоксі незручні —
натискання не завжди спрацьовує по всій видимій площі іконки.

Обгорнув кнопку видалення в `AlertDialog` (той самий компонент, що й скрізь інде в застосунку) —
клік відкриває підтвердження «Видалити фото?» / «Цю дію не можна скасувати», сам виклик
`deletePhotoAction` тепер лише в `AlertDialogAction`. Успішне видалення закриває обидва діалоги
разом (зовнішній лайтбокс розмонтовує вкладений `AlertDialog`), помилка лишає підтвердження
відкритим з тостом — той самий патерн, що вже в `RemoveParticipantButton`.

Кнопки прев'ю/наступне/видалити/закрити піднято з `size="icon-sm"` (28px) до `size="icon"`
(32px) і збільшено відступ між ними (`gap-1` → `gap-2`) — точна причина «не всюди спрацьовує
клік» не відтворювана без живого браузера, але замалий hit-target і тісні сусідні кнопки — це
найбезпечніший, завжди корисний напрямок фіксу.

`tsc --noEmit`, `npx vitest run` (897 тестів, +2 нових на підтвердження/скасування видалення,
решта delete-flow тестів оновлено під новий крок підтвердження), `npm run lint`,
`npm run build` — усі чисті.

## 2026-08-10 — Ще два флейкі-тести тієї самої породи (`getByRole("option")` без очікування)

Під час стрес-тестування повним `vitest run` спіймав живий провал у `matches-filters.test.tsx` і
`link-player-control.test.tsx` — обидва вже були згадані в записі 2026-08-06 як «того самого
різновиду флаку», але не виправлені тоді. Справжня причина конкретніша за просто «паралельне
навантаження»: `<Select>`-дропдаун рендерить опції в портал, що монтується асинхронно вже ПІСЛЯ
кліку на тригер, а не синхронно в його межах — голий `getByRole("option", ...)` одразу після
кліку на комбобокс іноді програвав цю гонку, як у `tournament-roster.test.tsx`, де аналогічне
місце вже правильно використовувало `findByRole` (авто-повторюваний запит).

Фікс: замінено голий `getByRole("option"/"Пошук…", ...)` на `await screen.findByRole(...)`/
`findByPlaceholderText(...)` у трьох місцях (`matches-filters.test.tsx`,
`link-player-control.test.tsx` ×2) — там, де опція справді запитується одразу після відкриття
дропдауна, без попереднього `findBy`/`waitFor`, що вже підтвердив би, що дропдаун відкрився.

Окремо — `tournament-roster.test.tsx`'s "adds every picked player…" (третя спроба з 2026-08-05):
одноразовий діагностичний тест підтвердив, що це не «revert» і не логічна регресія — оптимістичне
додавання таки комітиться і лишається, просто React-у під навантаженням іноді треба довше за
3000мс, щоб фактично відрендерити `startTransition`-оновлення. Підняв `findByText` таймаут до
8000мс (і `it()`-таймаут до 10000мс, бо дефолтний 5000мс Vitest інакше вбив би тест раніше) поверх
уже доданого `retry: 2`.

`npx vitest run` — два повних прогони поспіль чисті (895/895 обидва рази).

## 2026-08-10 — Фікс: флейкі-тест tournament-roster.test.tsx (третя спроба)

Два попередні заходи (2026-08-05: `findBy` timeout 1000→3000мс; продовження того ж дня:
`userEvent.setup({ delay: null })`) не прибрали флак повністю — під повним `vitest run` тест
зрідка все ще падав. Логіка компонента (`useOptimistic`) сама по собі коректна й оновлює UI
синхронно в межах transition — залишок флаку підтверджено суто середовищний: п'ять реальних
взаємодій поспіль означає п'ять `act()`/мікротаск точок, кожна з яких може розтягнутись під
навантаженням від паралельних воркерів тестів, не пов'язаним із кодом компонента.

Фікс: `{ retry: 2 }` саме для цього тесту (`tests/components/admin/tournament-roster.test.tsx`) —
стандартний інструмент Vitest для підтвердженого середовищного флаку: справжня регресія в логіці
падала б на кожній спробі, а не зрідка під навантаженням.

## 2026-08-10 — Клієнтське стиснення фото перед завантаженням (менше місця в R2, швидший аплоад)

Оригінали фото зберігались у R2 байт-у-байт, без жодного стиснення — телефонний оригінал (типово
3-8 МБ, до 4000+ px по довшій стороні) летів у сховище як є. R2 тариф із безкоштовним egress і
~10 ГБ безкоштовного зберігання робить це не гострою проблемою по грошах, але довший файл — це і
довше завантаження на мобільному зв'язку на кортах.

Новий `compressPhotoFile` (`src/lib/image-compress.ts`) — Canvas у браузері, викликається в обох
`PhotoUploadDialog` і `NewsPhotoField` одразу перед presign+PUT (не на сервері — лишається той
самий принцип «браузер → R2 напряму», без нової інфраструктури): обмежує довшу сторону
2560px і перекодовує в WebP з якістю 90%. Жодна невдача на цьому шляху (API не підтримується,
помилка декодування/кодування, чи «стиснений» результат виявився не меншим за оригінал) не блокує
завантаження — тихо повертає оригінальний файл незмінним, це суто оптимізація розміру, ніколи не
обов'язковий крок.

`tsc --noEmit`, `npx vitest run` (895 тестів, +9 нових на `compressPhotoFile` — непідтримуване
середовище, помилка декодування, недоступний canvas-контекст, провал кодування, «стиснений»
результат не менший за оригінал, коректне обмеження довшої сторони, без апскейлу дрібних фото),
`npm run lint`, `npm run build` — усі чисті.

## 2026-08-10 — Фікс: справжня причина «дригання» — горизонтальний скрол у списку файлів

Попередній фікс (нижче) прибрав N-кратний перерендер сторінки, але то виявилось не єдиною
причиною «дригання» — за скріншотом користувача, під час завантаження в `PhotoUploadDialog`
з'являвся горизонтальний скрол (зі стрілками ◀▶) прямо під рядком файлу, а іконка-спінер
«скакала». Причина класична для flexbox: `<span className="truncate">{item.file.name}</span>`
без `min-w-0` не стискається нижче природної ширини вмісту в flex-рядку, тож рядок (а разом з ним
і список, у якого `overflow-y-auto` за специфікацією CSS автоматично вмикає `overflow-x: auto`
теж) розтягувався ширше за діалог — звідси й спурний горизонтальний скрол, і візуальне
«скакання» спінера, притиснутого до правого краю цього рядка.

Фікс: додано `min-w-0` до `<span className="truncate">` в `src/components/admin/photo-upload-dialog.tsx`.

Заразом додав підказку про обмеження (формат/МБ/довжина назви файлу) біля кнопки вибору файлу і в
`PhotoUploadDialog`, і в `NewsPhotoField` — новий спільний `PHOTO_UPLOAD_HINT` і
`MAX_PHOTO_FILENAME_LENGTH` у `src/lib/validation/photo.ts`, щоб текст не розходився з тим, що
реально перевіряється.

`tsc --noEmit`, `npx vitest run` (886 тестів, +3 нових — regression guard на `min-w-0`, підказка
в обох діалогах), `npm run lint`, `npm run build` — усі чисті.

## 2026-08-10 — Фікс: «дригане» завантаження фото турніру (спінер блимав під час пакетного аплоаду)

Під час завантаження кількох фото в `PhotoUploadDialog` сторінка турніру (де діалог і відкритий,
поверх галереї) виглядала «дригано» — індикатор завантаження то зникав, то з'являвся знову.
Причина: `confirmPhotoUploadAction` викликається окремо для КОЖНОГО файлу (`Promise.all`), і
кожен виклик сам робив `revalidatePath('/tournaments/${tournamentId}')` — а ревалідація шляху,
який зараз відкритий, за поведінкою Server Function одразу перерендерює всю сторінку. Пакет із
N фото — N перерендерів поспіль, поки діалог лишається відкритим.

Прибрано цю ревалідацію з `confirmPhotoUploadAction` (лишились лише `/gallery` і
`/gallery/[id]` — вони не спричиняють живий флеш, бо адмін не на них у момент завантаження);
натомість `PhotoUploadDialog` сам викликає `router.refresh()` один раз, коли весь
`Promise.all`-пакет завершився. Детальніше — `docs/PHOTOS.md`.

`tsc --noEmit`, `npx vitest run` (884 тести, +1 новий — підтверджує рівно один `router.refresh()`
на пакет із 2 фото замість двох), `npm run lint`, `npm run build` — усі чисті.

## 2026-08-10 — Додано фото-обкладинку до новин

Форма «Додати новину» мала лише заголовок і текст — жодного способу проілюструвати новину фото.

`NewsPost` отримав nullable `photoKey` (R2-ключ, той самий формат, що й `Photo.key` — не окрема
таблиця-галерея, бо новина має щонайбільше одну обкладинку). Завантаження — той самий принцип
«браузер → R2 напряму», що й у фото турнірів (`docs/PHOTOS.md`), але влаштований інакше: адмін
обирає файл ще у формі створення посту, коли рядка `NewsPost` в БД ще нема, тож `NewsPhotoField`
(`src/components/admin/news-photo-field.tsx`) одразу пресайнить (новий
`src/app/api/news/photo-presign/route.ts`, без `tournamentId`) і вантажить файл у R2 ще до сабміту
форми, а `photoKey` їде далі як звичайне приховане поле форми — `createNewsPostAction`/
`updateNewsPostAction` (`src/lib/actions/news.ts`) просто записують його, без окремого «confirm»
Server Action. Заміна/видалення старого фото — best-effort видалення R2-об'єкта, той самий підхід,
що й `deletePhotoAction`.

Фото показується скрізь, де є новина: обкладинкою в `NewsCard` (головна, `/news`), на сторінці
самого посту (`/news/[id]`), і мініатюрою в адмінському списку (`/admin/news`).

`sanitizeFileName` виніс з `presign/route.ts` у `src/lib/r2.ts` (тепер потрібен двом presign-роутам).

`tsc --noEmit`, `npx vitest run` (883 тести, +19 нових: `NewsPhotoField` — валідація типу/розміру,
presign+PUT флоу, попередній перегляд і видалення існуючого фото; `createNewsPostAction`/
`updateNewsPostAction`/`deleteNewsPostAction` — прийняття/заміна/видалення/best-effort очищення
R2, відхилення чужого ключа; `NewsCard`/`NewsDialog` — показ обкладинки), `npm run lint`,
`npm run build`, smoke-перевірка `npm run dev` — усі чисті.

## 2026-08-10 — Новини тепер розгортаються інлайн замість переходу на окрему сторінку

І на головній, і в списку «Новини» (`/news`) текст новини завжди обрізався (`line-clamp-4`) без
жодного способу дочитати її, не переходячи на `/news/[id]` — окрема сторінка для одного абзацу
тексту.

Новий `NewsCard` (`src/components/news-card.tsx`, спільний для головної та `/news`) додає кнопку
«Читати повністю»/«Згорнути» під текстом, що знімає/повертає `line-clamp-4` без навігації.
Кнопка виникає, лише коли текст справді обрізаний — виявляється порівнянням
`scrollHeight`/`clientHeight` клампнутого параграфа через `useLayoutEffect` (після гідратації),
а не заданою наперед довжиною тексту. Заголовок як і раніше веде на `/news/[id]` для прямого
посилання на новину. `src/app/page.tsx` і `src/app/news/page.tsx` тепер використовують один
компонент замість двох майже ідентичних копій верстки картки.

`tsc --noEmit`, `npx vitest run` (870 тестів, +4 нових на `NewsCard`: посилання заголовка,
підпис автора, відсутність кнопки без переповнення, розгортання/згортання з реальним
переповненням), `npm run lint`, `npm run build`, smoke-перевірка `npm run dev` (SSR обох
сторінок з `line-clamp-4` у розмітці) — усі чисті.

## 2026-08-10 — Фікс: «1 балів» на профілі гравця — не застосовувалась українська плюралізація

Кількість очок SET.club на картці рейтингу (`RatingCard`, `src/app/players/[id]/page.tsx`)
завжди підписувалась незмінним словом «балів», навіть для 1 очка — мало бути «1 бал»,
«2 бали», «5 балів» за тим самим правилом, що вже застосовується скрізь інде (`pluralizeUk`).

Додано `POINT_FORMS: ["бал", "бали", "балів"]` (`src/lib/pluralize.ts`), підпис тепер рахується
через `pluralizeUk(setClub.points, POINT_FORMS)` замість жорстко зашитого «балів».

`tsc --noEmit`, `npx vitest run` (866 тестів; один відомий флейкі-тест
tournament-roster.test.tsx впав під паралельним навантаженням, пройшов в ізоляції — не
регресія), `npm run lint`, `npm run build` — усі чисті.

## 2026-08-10 — Бейдж «Знявся» тепер «Знявся з матчу» / «Знялась з матчу» (з урахуванням статі)

Голе слово «Знявся» на бейджі (`MatchSummary`, `ResultTile`) не пояснювало, що саме гравець
«зняв», і завжди стояло в чоловічому роді незалежно від статі гравця, що знявся.

`gender` додано до вибірки гравця в `matchWithDetailsInclude`
(`src/lib/queries/matches.ts`) — раніше зовсім не фетчився для матчів. Новий хелпер
`retiredLabel` (`src/lib/player-display.ts`) обирає «Знявся з матчу» чи «Знялась з матчу» за
статтю гравця сторони, що програла (саме вона знімається — переможця в `retired`-матчі завжди
вказує адмін явно, той самий принцип, що й у фіксі рахунку очок від 2026-08-09). Для пари
(DOUBLES) чи невідомої статі — той самий чоловічий за замовчуванням, що був і раніше. Технічну
поразку (`walkover`) текст не зачіпає — там завжди зрозуміло, хто саме не вийшов.

Довелось так само підправити `optimisticCreate` у `src/components/admin/tournament-matches.tsx`
(конструює тимчасовий `MatchPlayer` з `roster`, у якого немає поля `gender`) та тестові фікстури
(`tests/components/admin/tournament-matches.test.tsx`), щоб типи зійшлись після розширення
вибірки гравця.

`tsc --noEmit`, `npx vitest run` (866 тестів, +3 нових на узгодження зі статтю), `npm run lint`,
`npm run build` — усі чисті.

## 2026-08-10 — Плитки «Останні результати» на головній тепер позначають «Знявся»/«Технічна поразка»

`ResultTile` (`src/components/results-carousel.tsx`, стрічка «Останні результати» на головній)
показував лише рахунок сетів — на відміну від `MatchSummary`, який уже мав бейдж «Знявся»/
«Технічна поразка», тут для матчу, завершеного зняттям гравця, глядач бачив лише голий
(часто нетипово короткий чи нульовий) рахунок без жодного пояснення, чому він не схожий на
звичайний рахунок сету.

Додано той самий бейдж (`Badge variant="warning"`, «Знявся» для `match.retired`, «Технічна
поразка» для `match.walkover`) поруч із рахунком, з `flex-wrap`, щоб не ламати вузьку (w-44)
плитку каруселі.

`tsc --noEmit`, `npx vitest run` (864 тести, +3 нових на бейдж — retired/walkover/звичайний
матч), `npm run lint`, `npm run build` — усі чисті.
...
рахувало переможця з самого рахунку сету (`determineSetWinner`), ігноруючи явно вказаний
`winnerSide` — навіть коли матч завершено як "Знявся" (`retired: true`). А для `retired`-матчу
рахунок сету НЕ мусить формувати легальний/послідовний результат (гравець міг знятись
будь-якої миті) — і `saveScoreAction` саме тому просить адміна вказати переможця вручну явно
("хто не знявся"), а не вираховує його з рахунку. Якщо записаний "сет" виявлявся нічийним
(0:0) чи взагалі суперечив вибору адміна, переможець отримував 0 очок замість належних 2 —
і в основній "Таблиці", і в "Додатковій групі", і в "Підсумковій таблиці" (усі три джерело
очок з того самого коду).

Фікс: `computeMatchPoints` отримав третій параметр `retired` — коли `true`, завжди віддає
плоский рахунок 2-0 на користь `winnerSide`, той самий рахунок, що й для walkover, і ніколи не
намагається вирахувати переможця з (можливо неповного/нелегального) рахунку сету. `retired`
довелось додати у вибірку (`select`) усіх запитів, що годують `computeMatchPoints`
(`getIndividualRows`, `fetchDoublesMatches`, GROUPS12's mini-group query) — раніше поле взагалі
не фетчилось для цієї мети.

`tsc --noEmit`, `npx vitest run` (858 тестів, +6 нових на цей клас бага — singles/doubles,
основна таблиця/кастомна група, звичайний і нічийний recorded-сет), `npm run lint`,
`npm run build` — усі чисті.

## 2026-08-09 — Фікс: матч з іншого раунду інфлював лічильник матчів вбудованої групи

Продовження серії round-scoped фіксів для "Додаткових груп": ця ж діра лишалась у вбудованих
групах 1-6 ("За групами") і в "За сіяністю" (Gold/Silver) — `buildScopedSinglesRows`/
`buildDoublesGroup` для них НІКОЛИ не отримували `roundFilter` (немає єдиної очікуваної мітки
раунду, яку могла б задати вбудована група — коментар навмисно це пояснював), тож зараховували
БУДЬ-який матч між двома учасниками групи, незалежно від раунду. Двоє гравців "Групи B", що
пізніше зустрілись ще раз у плей-офф (напр. "За 5 місце") чи в кастомній "Додатковій групі" —
той повторний матч тихо додавав +1 до "Матчів" обом у таблиці "Групи B", хоча реально групових
матчів було на один менше.

Фікс — не строгий allowlist (як для кастомних груп, де немає одного очевидного значення), а
denylist: матч без раунду чи з нерозпізнаним вільним текстом і далі рахується (щоб вручну
доданий груповий матч не випав), але матч з розпізнаваним ЧУЖИМ раундом — curated плей-офф
(`isPlayoffRound`) або назвою іншої кастомної групи — виключається. `buildGroups12PlayoffTable`'s
виклик для міні-групи 9-12 тепер явно передає `MINI_GROUP_ROUND` як `roundFilter` (раніше
покладався на "без фільтра", але `MINI_GROUP_ROUND` сам входить у `PLAYOFF_ROUNDS` — без явного
фільтра новий denylist помилково виключив би всі її власні матчі).

`tsc --noEmit`, `npx vitest run` (852 тестів, +3 нових на цей клас витоку — singles built-in
group, singles vs. custom group, doubles built-in group), `npm run lint`, `npm run build` — усі
чисті.

## 2026-08-09 — Три фікси "Підсумкової таблиці" для турнірів поза GROUPS_12_PLAYOFF

Продовження попередньої зміни (загальна "Підсумкова таблиця" для будь-якого турніру з реальними
плей-офф матчами) — за конкретним турніром користувач знайшов три проблеми:

1. **Місця 7-10 не проставлялись.** Турнір мав власну "Група за 7-10 місце" (міні-раундробін
   нижніх фіналістів, аналогічний вбудованому 9-12 місцю GROUPS_12_PLAYOFF, але з довільною
   назвою й діапазоном) — `buildGeneralPlacedTable` вмів резолвити місця лише з реальних
   Фінал/За 3/5/7/9/11 матчів, не з кастомних груп-раундробінів. Тепер після резолву decisive-
   матчів функція перебирає `customGroups`: якщо група ще не вирішена, всі її учасники ще без
   місця, і її власний round robin (той самий `buildScopedSinglesRows`, що й для звичайних
   "Додаткових груп") завершений — заповнює наступний блок місць одразу за вже вирішеними, у
   порядку її власного `sortRows`. Кілька таких груп заповнюють суміжні блоки без колізій.
2. **"Правила формату" показували опис GROUPS_12_PLAYOFF для звичайного турніру.**
   `Groups12PlayoffInfoButton` (`src/app/tournaments/[id]/page.tsx`) вмикався за самою наявністю
   `standings.placedTable` — тепер, коли `placedTable` будується і для не-GROUPS12 турнірів,
   кнопка показувала жорстко захардкоджені правила "4 групи по 3 + плей-офф" на турнірі з
   геть іншою структурою. Новий дискримінатор `TournamentStandingsResult.isGroups12Playoff`
   (`true` лише коли реально задетектовано `buildGroups12PlayoffTable`) — кнопка тепер умикається
   саме за ним.
3. **Кубок за 1-ше місце не показувався** — наслідок пункту 1: `PlacedTournamentStandings`'s
   `complete` вимагає місця для ВСІХ рядків; поки 7-10 лишались `null`, `complete` був `false`
   назавжди, тож трофей за 1-ше місце (умова `complete && place === 1`) ніколи не з'являвся,
   навіть коли фінал давно зіграно. Фіксується автоматично разом з пунктом 1.

`tsc --noEmit`, `npx vitest run` (849 тестів), `npm run lint`, `npm run build` — усі чисті.

## 2026-08-09 — "Підсумкова таблиця" тепер і для турнірів поза GROUPS_12_PLAYOFF

Досі "Підсумкова таблиця" (комбінована таблиця 1..N за реальним місцем, а не живим
рахунком перемог) існувала лише для 12-гравцевого рандомайзера "4 групи по 3 + плей-офф" -
детектувалась структурно за наявністю матчів "Група за 9-12 місце". Турнір, зібраний вручну
(вбудовані групи A/B + власноруч створені плей-офф матчі Фінал/За 3/За 5/За 7 місце), такого
детектора не мав — і "Підсумкова таблиця" просто не з'являлась, хоча всі місця вже реально
вирішені.

Новий `buildGeneralPlacedTable` (`src/lib/tournament-standings.ts`) — загальний варіант:
резолвить місця з будь-яких завершених матчів, чий `round` входить у `PLACEMENT_ROUND_RANKS`
(Фінал/За 3/5/7/9/11 місце), через уже наявний `resolveDecisivePlacements` (той самий, що й
рейтинговий рушій Set Club використовує для нарахування очок). Свідомо НЕ використовує повний
`resolvePlacements` з round-robin fallback'ом — гравець, чиє місце не вирішив реальний матч,
лишається `null` ("—" в UI), а не отримує здогадку за рахунком групового етапу (два гравці з
різних груп, які не грали одне з одним, інакше зрівнювались би напряму). GROUPS_12_PLAYOFF-
специфічна таблиця (з точнішим 9-12 міні-груповим розподілом) і далі має пріоритет, коли
детектується; загальна — лише fallback, коли є хоч один реальний матч з curated-раундом.

`tsc --noEmit`, `npx vitest run` (846 тестів), `npm run lint`, `npm run build` — усі чисті.

## 2026-08-09 — Фікс: неможливо було створити матч-плейсхолдер без гравців, тільки з раундом

`matchFormSchema`'s `playerIdList.min(0)` навмисно дозволяє порожню сторону (плейсхолдер під
матч плей-офф, гравці якого ще не визначені), але через `create-match-dialog.tsx` це фактично
ніколи не працювало: Base UI `Select` для вибору гравця (не `multiple`) завжди рендерить прихований
`<input>` з поточним значенням — навіть коли нічого не обрано, це `value=""`, а не відсутнє поле.
Тож `formData.getAll("sideAPlayerIds")` повертав `[""]` замість `[]`, і Zod валив увесь сабміт
повідомленням на кшталт "String must contain at least 1 character(s)".

`createMatchAction`/`updateMatchAction` (`src/lib/actions/matches.ts`) тепер фільтрують порожні
рядки з `sideAPlayerIds`/`sideBPlayerIds` перед валідацією (`nonEmptyFormValues`) — порожній слот
коректно перетворюється на відсутність гравця, не на "порожній playerId". Нові тести: створення
й редагування матчу з обома сторонами незаповненими (лише раунд) більше не валиться.

`tsc --noEmit`, `npx vitest run` (843 тестів), `npm run lint`, `npm run build` — усі чисті.

## 2026-08-09 — Порожня "Додаткова група" тепер видима і в "Таблиці", не лише в дропдауні раунду

Можна створити "Додаткову групу" без учасників (наприклад, щоб спершу мати назву раунду для
`MatchDialog`, а склад додати пізніше). Раніше `getTournamentStandingsRows`
(`src/lib/tournament-standings.ts`) повністю ховала таку групу з вкладки "Таблиця" —
`customGroups.map(...).filter((g) => g !== null)` відкидав будь-яку групу з нульовим
`members.length` ще до рендеру. Прибрав цей фільтр (і в singles-, і в doubles-гілці) — порожня
група тепер завжди рендериться як своя секція з плейсхолдером "Матчів ще немає.", доки в неї не
додадуть учасників через `EditTournamentGroupDialog`.

`tsc --noEmit`, `npx vitest run` (841 тестів), `npm run lint`, `npm run build` — усі чисті.

## 2026-08-09 — "Втішний півфінал" у дропдауні "Раунд"

Раніше `CONSOLATION_SEMIFINAL_ROUND` ("Втішний півфінал") використовувався лише як внутрішня
мітка автогенерованих матчів рандомайзера "4 групи по 3 + плей-офф"
(`src/lib/groups12-playoff-bracket.ts`) — вручну обрати його в `MatchDialog` не можна було. Тепер
доданий у `BRACKET_ROUND_PICKER_OPTIONS` (`src/lib/playoff-rounds.ts`), одразу після "1/4" — той
самий порядок, що й у `PLAYOFF_DISPLAY_ORDER`. Ця мітка й раніше була звільнена від обмеження
"один матч на раунд" (кілька втішних півфіналів — легальний стан), тож додавання в пікер нічого
там не ламає. `create-match-dialog.tsx`'s `ROUND_SELECT_LABELS` отримав відповідний запис для
коректного відображення обраного значення.

## 2026-08-09 — Фікс: редагування назви турніру валилось, якщо формат заблокований

Коли в турнірі вже є матчі, `TournamentForm` блокує пікер "Формат" (`disabled`), щоб його не
можна було змінити. Але вимкнений (`disabled`) HTML-контрол взагалі не потрапляє в `FormData` —
тож будь-яке збереження форми (навіть просто зміна назви) відправляло `format` як `null`, і
Zod валив увесь сабміт з "Invalid option: expected one of...". Фікс:
`src/components/admin/tournament-form.tsx` тепер рендерить прихований `<input name="format">`
з поточним значенням, коли пікер заблокований, а сам `Select` у цьому випадку `name` не несе —
той самий патерн умовного `name`, що вже є в `create-match-dialog.tsx`'s раунд-пікері. Новий
regression-тест сабмітить форму із заблокованим форматом і перевіряє, що `format` таки дійшов
до `updateTournamentAction`.
...
вручну набирати назву групи в полі "Інше…", ризикуючи опискою, яка тихо зламає прив'язку матчу
до групи. `MatchDialog` (`src/components/admin/create-match-dialog.tsx`) отримав новий опційний
пропс `customGroupNames: string[]` — назви кастомних груп турніру рендеряться окремою секцією
"Додаткові групи" в дропдауні "Раунд", між "Матч за місце" і "Інше…". Вибір такого пункту
виставляє `Match.round` рівно у назву групи — саме те, що потрібно попередньому фіксу для
зарахування матчу.

Назва, що збігається з уже наявним curated-раундом (напр. якщо кастомну групу назвали
"За 7 місце"), у новій секції не дублюється — цей випадок і так вже працює через існуючий
curated-пункт (однаковий рядок). `deriveRoundSelection` тепер розпізнає збіг з кастомною групою
так само, як curated-раунд — редагування вже прив'язаного матчу показує групу вибраною в
дропдауні, а не у вільному тексті.

`src/components/admin/tournament-matches.tsx` прокидає список назв (значення вже наявної Map
`customGroupNames: Map<number, string>`, нічого нового не фетчили) в обидва виклики
`MatchDialog` — і "Додати матч", і редагування кожного рядка.

`tsc --noEmit`, `npx vitest run` (824 тестів), `npm run lint`, `npm run build` — усі чисті.

## 2026-08-09 — Фікс: матч зі старої групи витікав у "Додаткові групи" за самим лише складом

За скріншотом таблиці турніру: гравці з `Група B` (наприклад Очеретенко й Теребіж), додані
пізніше в кастомну "Додаткову групу" ("За 7 місце"), бачили у ній старий рахунок з Групи B,
хоча жодного матчу "За 7 місце" між ними ще не було. Причина —
`buildScopedSinglesRows`/`buildDoublesGroup` у
[src/lib/tournament-standings.ts](../src/lib/tournament-standings.ts) фільтрували матчі лише за
тим, що ВСІ гравці матчу є учасниками групи, ігноруючи `Match.round` — тож будь-який старий матч,
чиї учасники згодом опинилися разом в іншій кастомній групі, зараховувався туди теж.

Фікс: для секції "Додаткові групи" (і тільки для неї — вбудовані групи 1-6/"За сіяністю" не
чіпали) додано другу умову: `match.round === назва_групи`. Матч зараховується в кастомну групу,
лише якщо адмін явно вказав раунд, що збігається з назвою групи. Додав `round` у вибірку матчів
(`CompletedMatchRow`, `DoublesMatchRow`), новий необов'язковий параметр `roundFilter` у
`buildScopedSinglesRows`/`buildDoublesGroup`/`buildSinglesGroup`.

Нові регресійні тести (singles + doubles): матч, зіграний повністю всередині вбудованої групи,
не повинен потрапляти в кастомну групу, навіть якщо обидва/усі гравці матчу згодом стали членами
цієї кастомної групи. `tsc --noEmit`, `npx vitest run` (819 тестів), `npm run lint`,
`npm run build` — усі чисті.
...
- **`revalidatePath` не покривав нові роути** — `confirmPhotoUploadAction`/`deletePhotoAction`
  (`src/lib/actions/photos.ts`) ревалідували лише `/tournaments/${id}`, тож `/gallery` (Full Route
  Cache, без динамічних API) міг довго показувати застарілий список/обкладинку чи навіть зламане
  зображення після видалення обкладинкового фото. Додано ревалідацію `/gallery` й
  `/gallery/${tournamentId}` в обох діях.
- **Подвійний запит турніру на `/gallery/[id]`** — `generateMetadata` і сама сторінка обидва
  викликали `getTournamentById`. Обгорнув саму функцію (`src/lib/queries/tournaments.ts`) в
  `React.cache()` — задокументований патерн Next.js для дедуплікації запиту на один рендер;
  заодно фіксить той самий подвійний запит і на `/tournaments/[id]`, де його не чіпали.
- **Тест `getTournamentsWithPhotos` перевіряв лише аргументи виклику Prisma**, не сам результат —
  додано перевірку, що обкладинка й лічильник дійсно доходять до виклика без спотворень.
- **Нуль тестів на `photo-lightbox.tsx`/`photo-upload-dialog.tsx`** (client-компоненти з реальною
  логікою — видалення/навігація, валідація файлів/мультизавантаження) — додано
  `tests/components/photo-lightbox.test.tsx` і `tests/components/admin/photo-upload-dialog.test.tsx`,
  за зразком `news-dialog.test.tsx`/`randomize-matches-button.test.tsx`. Дрібний супутній фікс:
  файловий `<input>` не мав доступного імені (`aria-label="Файли фото"`) — і для скрін-рідерів, і
  щоб тест міг знайти елемент через `getByLabelText`.

## 2026-08-08 — Розділ "Фото" в меню + продакшн-фікси R2-аплоаду

Продовження фічі фото турнірів (`docs/PHOTOS.md`). Два фікси, виявлені під час першого реального
завантаження на проді:
- CSP блокував сам аплоад — `img-src`/`next/image` покривали лише **показ** фото з `r2.dev`, але
  presigned PUT браузер шле напряму на інший хост, `*.r2.cloudflarestorage.com`, якого не було в
  `connect-src`. Додано в `next.config.ts`.
- R2-бакет не мав CORS-політики — без неї browser блокує сам preflight PUT-запиту (окрема від
  CSP перевірка). Виставлено вручну через Cloudflare UI (не через S3 API — токен з правами Object
  Read & Write не має прав міняти bucket-level налаштування).

Плюс новий розділ меню "Фото" (`src/lib/site.ts`'s `NAV_LINKS`): `/gallery` — список турнірів, у
яких є хоч одне фото (картка з обкладинкою-мініатюрою й лічильником), `/gallery/[id]` — повна
галерея конкретного турніру (той самий `PhotoLightbox`, що вже був на сторінці турніру). Спільний
Prisma-запит виніс у новий `src/lib/queries/photos.ts` (`getPhotosByTournament`,
`getTournamentsWithPhotos`), `src/components/tournament-gallery.tsx` переписано на його виклик
замість інлайн-запиту, щоб не дублювати логіку. Перегляд лишається публічним, видалення — тільки
адмінам; завантаження нових фото — і далі тільки зі сторінки турніру.

## 2026-08-08 — Аудит змін за день: 2 дрібні знахідки, обидві виправлено

На прохання користувача — перевірка усіх змін цього дня (11 комітів: withdrawal/walkover,
нікнейм, ребрендинг, плаваюче SET.club-вікно, фікс "Обнулити турнір", рефактор рангу/рейтингу на
картках, приховання SideRatings, фікс "За сіяністю") на можливі баги. Основна ризикована частина —
транзакційна логіка `withdrawParticipantAction` і walkover-виключення з рейтингу/статистики/h2h —
пройшла перевірку без знахідок (перехресно узгоджена в 6+ незалежних місцях: bracket-advancement,
tournament-standings, player-stats/stats.ts, head-to-head, ratings-data, рандомайзери). Дві дрібні,
не функціональні знахідки:

- `src/lib/tournament-standings.ts` — коментар до фікса "За сіяністю" (див. вчорашній запис) хибно
  стверджував, що звичайний рандомайзер "За групами" використовує `seed` для анкерування груп
  (так само як GROUPS_12_PLAYOFF) — насправді `assignUngroupedToGroups`
  (`src/lib/randomize-pairs.ts`) працює виключно з уже призначеним `group`, зовсім не торкаючись
  `seed`. Сама поведінка коду лишилась правильною (підтверджено користувачем окремо) — виправлено
  лише коментар, щоб не вводити в оману.
- `src/components/admin/tournament-roster.tsx` — підтвердження зняття гравця з турніру
  узгоджувало прикметник/іменник лише для "1" проти "решти" (`"2 запланованих матчів"` замість
  правильного `"2 заплановані матчі"`) — замінено на `pluralizeUk` (те саме, що скрізь інде в
  проєкті), яке коректно розрізняє 1 / 2-4 / 5+.

## 2026-08-08 — Таблиця турніру: не показувати "За сіяністю" для турніру, сформованого "За групами"

Одиночний турнір, сформований жеребкуванням "За групами", теж використовує `seed` — але лише як
один анкер на групу (щоб розкидати сіяних по різних групах), а не як окремий "хто сильніший"
розподіл. Через це таблиця вже показувала зайву, беззмістовну секцію "За сіяністю" (Gold — самі
анкери, які між собою жодного матчу не грали; Silver — по суті майже всі інші, дублікат загальної
таблиці) поруч із нормальною "За групами". Той самий випадок, що вже був виправлений для
GROUPS_12_PLAYOFF — тепер узагальнено на будь-який турнір, де активний груповий розподіл.

- `src/lib/tournament-standings.ts` — `hasSeeds` тепер додатково враховує `!hasGroups` (було лише
  `!groups12Playoff`).

## 2026-08-08 — Сховано число SET.club-балів справа на картці запланованого матчу

Користувач передумав одразу після впровадження попереднього пункту: число замість рахунку
(колишній `SideRatings`) вирішили поки прибрати з UI, лишивши можливість повернути пізніше. Ранг
"(#N)" біля імені та відсоток шансів на перемогу — не чіпали, лишаються як є.

- `src/components/match-summary.tsx` — видалено компонент `SideRatings` і `ratingDisplay`/
  `showRatings` у `MatchSummary`/`SideRow`; для запланованого матчу колонка рахунку тепер знову
  просто порожня, як до фічі. `SideRow.ratingDisplay` лишився як загальний (нічим більше не
  задіяний) проп.
- Дані під капотом **не чіпали**: `buildMatchPreview` і далі рахує й повертає
  `preview.pointsByPlayerId` (SET.club-бали), просто ніхто в UI його зараз не читає — тож
  повернути показ можна буде, додавши назад лише JSX без нових запитів/розрахунків.
...
not defined`) на проді, хоч локальний `tsc`/`vitest` цього не ловили. Причина: `matches.ts` —
`"use server"`-файл — містив `export type { CascadeReset };`, зайвий ре-експорт (ніхто його з
`matches.ts` і не імпортував — `tournaments.ts` уже брав `CascadeReset` напряму з
`bracket-snapshot.ts`). Next.js дозволяє з `"use server"`-файлів експортувати лише async-функції,
але тип-експорт мав стиратись компілятором; замість цього turbopack-бандлер серверних дій для
`/admin/tournaments/[id]` (`_next-internal_..._page_actions_*.js`) згенерував зламане runtime-
посилання на неіснуюче значення `CascadeReset`, що кидало помилку одразу при завантаженні модуля —
ще до виклику самого `resetTournamentAction`. Той самий клас багу, що й попередній фікс білда
("Only async functions..."), але непіймана на цей раз, бо тип-only експорт не порушує ту
конкретну заборону — знайдено через `vercel logs` на проді, не через локальне відтворення.
- `src/lib/actions/matches.ts` — видалено мертвий `export type { CascadeReset };`.

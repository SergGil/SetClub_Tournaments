...
- `new.tsx`, `[id]/edit.tsx` — форма (`src/features/tournaments/tournament-form.tsx`, спільна),
  сегментовані пікери формату/статусу/покриття, дати як текст `РРРР-ММ-ДД` (без нативного
  date-picker — свідомо, щоб не додавати ще одну нативну залежність на цьому кроці; можна додати
  `@react-native-community/datetimepicker` пізніше).
...
- `mobile/` — весь Expo-проєкт (свій `package.json`/`node_modules`/`.gitignore` — останній вже
  ...
  чіпали).
- `mobile/src/lib/{api,auth-context,session-storage,query-client,permissions,rating-math}.ts`.
- `mobile/src/features/{tournaments,matches,players,news,menu,users,rating}/**`.
...
Готово:
- **tournaments** — повний CRUD, учасники, групи.
...
- **rating** — перемикач Теніс/Падел і Одиночний/Парний, список гравців за conservative
  rating/ordinal (`src/lib/rating-math.ts` — легке дублювання чистої математики з
  `src/lib/rating/{glicko2,openskill}.ts`, без залежності від бекенду).
- **news** — список, картка, CRUD (без завантаження фото з мобільного — див. нижче).
...
- Завантаження фото (новини, турніри) — presign-flow вже готовий на бекенді
  (`docs/MOBILE_API.md`), але image-picker + upload UI на мобільному ще не підключено.
- **Padel-дзеркало** для tournaments/matches — свідомо НЕ дубльовані окремі екрани (на відміну
  від бекенду, де кожен домен — окремий Server Action файл): дизайн-рішення — параметризувати
  наявні екрани перемикачем спорту (як уже зроблено в `rating.tsx`) замість дублювання
...
- `npx tsc --noEmit`, `npx expo-doctor` (21/21), `npm run lint` — чисто (крім двох
  попереджень/однієї помилки в незміненому шаблонному коді `themed-view.tsx`/
  `use-color-scheme.web.ts` — успадковано з `create-expo-app`, поза межами цієї роботи).
